...
| Versão | 0.6 — o recolhimento de callbacks virou `npm run receptor:recolher`, e a instrução anterior so funcionava em bash (D-191) |
| Data | 2026-09-02 (criado em 2026-08-17) |
...
| D-155 | **A demo mostra o texto de produção, e uma trava impede que ele vire produção sozinho.** A rota de consulta da Demo B envia texto livre ao cliente **sem parar em ninguém**, e a nota de rodapé afirma "revisada por um advogado do escritório". A frase é **falsa neste fluxo** — e **fica assim de propósito**: a demonstração existe para mostrar ao escritório como será o atendimento real, e em produção o texto passa por aprovação. Maquete tem de mostrar o rodapé da casa pronta. O que torna isso aceitável não é a intenção, é o contexto verificável: **destinatária única, que conduz a demo**, e o aviso `⚠️ DEMONSTRAÇÃO` no topo de toda resposta. Ninguém é levado a acreditar em revisão que não houve. **O risco não é a demo, é o esquecimento** — o dia em que o aviso sair e a frase ficar, o escritório afirma a cliente de verdade que um advogado leu o que nenhum advogado leu, em canal regido pelo Provimento 205/2021 da OAB. Por isso a trava de `demo/testar-fluxo-b.mjs` **não proíbe a frase: amarra a frase ao aviso.** Enquanto o texto afirmar revisão humana, o fluxo é obrigado a se declarar demonstração — a menos que exista nó de aprovação. Quem tirar o aviso para atender cliente real encontra o teste no caminho, e a escolha vira explícita: entra a aprovação da faixa **A3b** (D-142), ou sai a afirmação. **Esta rota não atende cliente real na forma atual** | Adotar, e não levar à produção sem o nó de aprovação | ✅ **Confirmada** (revisão externa 28/08; escopo ajustado pelo usuário 31/08) |
| D-156 | **A D-142 entrou no código, e a aprovação A3b passou a conferir o papel de quem aprovou.** Três mudanças juntas. **(1)** `A3` puro **deixou de existir** na lista de faixas: ferramenta que ainda o declare é recusada na carga, com a mensagem ensinando a escolher. Mantê-lo válido faria toda ferramenta anterior à D-142 seguir passando sem que ninguém decidisse de que lado ela cai — herdando, em metade dos casos, o lado mais permissivo. **(2)** **A3b exige papel de advogado ou sócio.** Até 31/08 o chassi conferia papel só em A4, e uma aprovação A3 assinada por estagiário passava — justamente na faixa do que sai do escritório e chega ao cliente. O código tratava a exigência como rito adiado para a Parte II; mas enquanto uma exigência está adiada, o comportamento em vigor é o permissivo, e a Regra 5 manda o contrário. Afrouxar depois é decisão registrada; afrouxar por omissão não é decisão nenhuma. **(3)** **A3a é recusada na carga enquanto o catálogo de gabaritos não existir** (`A3A_DISPONIVEL`), no espírito da D-140: ela dispensa aprovação porque um gabarito aprovado antes a substitui, e sem catálogo seria uma faixa que libera apoiada numa garantia que ninguém verifica. Até lá, comunicação externa é A3b. Quatro testes novos; o do estagiário foi rodado contra a etapa anterior e reprova lá | Adotar | ✅ **Confirmada** (revisão externa, 31/08) |
| D-157 | **O isolamento entre escritórios é do banco, em duas camadas que não se substituem.** **Chaves compostas** (migração 009): 30 referências passaram a apontar para o par `(inquilino_id, id)` — `cliente_id REFERENCES cliente(id)` garantia que o cliente existe, não que fosse do mesmo escritório. **Política por linha** (migração 010): a conexão declara em que escritório trabalha, e o banco devolve só aquilo; consulta que esquece o `WHERE inquilino_id` deixa de vazar. As duas são necessárias — conferência de chave estrangeira roda como dona da tabela e passa por cima da política, então só a 009 fica entre um bug e uma referência cruzada. **Falha fechada com preço declarado:** conexão sem escritório declarado não vê nada, e o sintoma é vazio, não erro; a defesa contra isso é `noInquilino(inquilino_id, …)` em `conexao.ts`, que exige o escritório como argumento. **Foi possível porque a migração 007 já separava `lex_dono` de `lex_app` por outro motivo** — política por linha não se aplica ao dono da tabela, e essa pré-condição já estava satisfeita por acidente de bom desenho. Provado com e sem a política: com, o escritório A vê 0 clientes do B; sem, vê 1 | Adotar | ✅ **Confirmada** (revisão externa, 01/09) |
| D-158 | **Aprovação vale UMA vez, neste escritório e nesta conversa.** `usado_em` e `sessao_id` na tabela, mais `inquilino_id` e `sessao_id` conferidos pelo chassi. O caso real do uso único não é o atacante: é a retentativa por timeout, que reapresenta a mesma aprovação e manda uma segunda mensagem ao cliente com a assinatura de um advogado que autorizou uma. Gastar é atômico e vive **fora** do chassi (`RegistroDeAprovacoes`), porque conferir "já foi usada?" e marcar "agora foi" são dois passos e entre eles cabe uma segunda chamada — o `UPDATE … WHERE usado_em IS NULL` resolve no banco o que um `if` na aplicação não resolve. A interface nasce **antes** da implementação, no caminho da `Auditoria`: faixa que exige aprovação sem registro configurado **recusa**, para que o marco 9 nasça com uso único obrigatório em vez de ganhá-lo depois | Adotar | ✅ **Confirmada** (revisão externa, 01/09) |
| D-159 | **A pseudonimização por HMAC fica para depois da apresentação — decisão do usuário, 01/09.** O pseudônimo sai de `SHA-256(nome)` sem segredo, e quem tiver a lista de partes dos autos consegue testar candidatos. HMAC fecha isso. **O adiamento é deliberado e tem motivo:** trocar o hash muda **todos** os pseudônimos, e com eles o instantâneo, os dois workflows e o nome gravado em `demo/listas/clientes.json` — a demo passaria a mostrar nomes diferentes dos já apresentados. É o único item da revisão com efeito visível na demonstração. **Retomar depois da apresentação ao escritório**, junto com a atualização de `clientes.json` | Adiar, e retomar | 🟡 **Proposta** |
| D-157 | **O servico de auditoria conecta como `lex_app`, nunca como o dono do banco.** O *append-only* tem duas camadas — um gatilho que vale ate para o dono (migracao 003) e a revogacao de UPDATE/DELETE para o papel da aplicacao (migracao 007). Conectar como `lex_dono` **nao quebraria teste nenhum**: o gatilho continuaria recusando e tudo pareceria certo, enquanto o desenho de duas camadas virava uma em silencio. A segunda camada existe justamente porque a primeira e um objeto do banco que quem tem DDL pode remover, e um `DROP TRIGGER` acidental numa migracao futura so seria descoberto no dia em que a prova fizesse falta. Por isso `conferirPapel()` roda na partida e papel errado e **erro de partida**, nao aviso | Adotar | 🟡 Proposta |
| D-158 | **O resumo da auditoria RECUSA dado pessoal antes do INSERT, e recusa em vez de limpar.** A tabela e imutavel por gatilho e por permissao, o que e exatamente o que se quer de uma prova e exatamente o que nao se quer de um vazamento: **dado pessoal que entra ali nao sai nunca mais** — nao ha apagamento seletivo, nao ha correcao, e um pedido de eliminacao sob a LGPD encontra uma tabela desenhada para recusa-lo. Dai as duas escolhas: a barreira fica **antes** do INSERT, unica posicao em que pode ficar (depois nao existe conserto, existe incidente), e ela **recusa** em vez de mascarar — mascarar em silencio ensinaria a proxima pessoa que da para mandar qualquer coisa porque "o resumo cuida disso", e um dia a expressao regular nao pegaria. Como `registrar` que lanca e chamada que nao acontece, tentar gravar um CNJ no resumo bloqueia a operacao. E severo de proposito. A mensagem de recusa **nao repete o que encontrou**, para nao escrever o dado no log do servidor | Adotar | 🟡 Proposta |
| D-159 | **`aprovacao_id` entra no evento de auditoria, no sucesso e na recusa.** A coluna existia desde a migracao 003 e o chassi nao a preenchia — achado 4 da revisao de 28/08. A pergunta que se faz depois de um ato de faixa A4 dar errado nao e "houve aprovacao?", e **"de quem foi a assinatura?"**; sem o campo a trilha responde a primeira e nao a segunda. A Regra 2 exige advogado IDENTIFICADO, e identificar na hora de decidir e esquecer na hora de registrar cumpre a metade da regra que nao serve para nada. Vale tambem na recusa: aprovacao de estagiario para A4 e negada, e o registro guarda **qual** aprovacao foi apresentada — que e o que permite investigar quem tentou assinar o que nao podia | Adotar | 🟡 Proposta |
| D-160 | **A consulta de trilha exige `inquilino_id`; `requisicao_id` sozinho atravessa inquilinos.** E o mesmo defeito que a revisao encontrou no comentario da migracao 005: chave composta impede **colisao**, nao **leitura**. O `requisicao_id` e um uuid, entao a colisao entre inquilinos e improbabilissima — e "improbabilissima" e propriedade estatistica, nao barreira. Tornar o inquilino parametro obrigatorio da funcao e o que impede a assinatura dela de um dia ser chamada sem ele | Adotar | 🟡 Proposta |
| D-161 | **`pg` entra como a primeira dependencia de producao do monorepo — ferramenta fala com o banco por `psql`, servico nao.** Ate aqui havia uma unica dependencia, e de desenvolvimento: o TypeScript. O `migrar.mjs` e o `conferir-regras.mjs` conversam com o PostgreSQL por `docker compose exec psql`, o que serve para ferramenta e **nao serve para servico**: um processo por gravacao de auditoria e inviavel quando toda chamada gera pelo menos uma. Os prazos sao curtos de proposito (5 s) — pool que espera indefinidamente transforma "auditoria fora do ar" em "a plataforma travou", e o segundo e pior, porque o primeiro recusa e avisa e o segundo pendura sem dizer nada. A D-77 quer bloqueio **com resposta** | Adotar | 🟡 Proposta |
| D-162 | **A prova do *append-only* deixa residuo permanente no banco, e isso e a propriedade, nao um defeito do teste.** O `conferir-regras.mjs` do marco 1 desfaz tudo com ROLLBACK; `conferir-auditoria.mjs` nao consegue, por construcao — **um teste de append-only que limpasse a si mesmo teria acabado de refutar o que veio testar**. As linhas ficam sob um inquilino de teste declarado, e `npm run banco:zerar` recria o banco quando incomodar | Adotar | 🟡 Proposta |
| D-163 | **A recusa deixa de ser um beco: quando o robô não pode responder, ele chama uma pessoa.** Prazo, prognóstico e pedido de atendimento humano passam a notificar um advogado no Telegram, com o nome do cliente, o motivo e a pergunta original. O destinatário é **advogado**, e não qualquer colaborador, porque as três perguntas que disparam o chamado são justamente as que só advogado responde (Regra 2) — estagiário recebendo "vou ganhar a causa?" só muda o beco de lugar. Sem advogado na lista, o gerador **recusa publicar**: um fluxo que diz "já avisei o escritório" sem ter a quem avisar é a promessa vazia da D-102 de volta | Adotar | 🟡 Proposta |
| D-164 | **O chamado acontece ANTES da resposta ao cliente, e a resposta muda conforme ele tenha chegado.** São três textos: o base, que não afirma aviso nenhum; o de sucesso, que diz "já avisei o escritório"; e o de falha, que diz que **não** conseguiu avisar e manda ligar. É a D-101 aplicada ao outro lado do balcão — lá era a tela do colaborador dizendo "enviado" antes do envio, aqui seria o cliente ouvindo "já avisei" antes do aviso. O nó do Telegram leva `retryOnFail` e **saída de erro**, nunca `continueRegularOutput`: engolir a falha produziria exatamente a frase falsa | Adotar | 🟡 Proposta |
| D-165 | **Prognóstico vira barreira em código, ao lado do prazo.** "Eu vou ganhar?" é a pergunta que mais interessa ao cliente e a que menos pode ser respondida por robô: errar para mais cria expectativa que vira reclamação na OAB, errar para menos faz o cliente desistir de direito que tinha, e **não existe resposta prudente automática** — até "as chances parecem boas" é opinião jurídica dada por quem não pode dar. Como no prazo (R-02), a recusa vem **antes do modelo**: o modelo bom recusaria, mas só se descobre qual modelo se tem depois que ele já respondeu | Adotar | 🟡 Proposta |
| D-166 | **A mensagem do cliente entra na notificação como citação, nunca como instrução (Regra 4).** Vai escapada em HTML, cortada em 500 caracteres e sob uma linha que diz ao colaborador, em letra clara, que aquilo é fala de cliente e não ordem ao assistente. O número do cliente vai **mascarado** — quem atende precisa saber quem é, não precisa do telefone inteiro numa tela que pode estar sendo projetada numa reunião | Adotar | 🟡 Proposta |
| D-167 | **Se o sistema oferece uma opção com um rótulo, ele é obrigado a aceitar aquele rótulo de volta.** O assistente listava os processos pela classe ("procedimento comum cível", "agravo de instrumento") e depois só aceitava resposta por número CNJ — o cliente respondeu exatamente o que fora oferecido e recebeu a mesma pergunta três vezes. Passam a valer três formas, da mais específica para a menos: o número, o rótulo (por palavras, sem acento e sem as que não distinguem nada) e a ordem da lista ("o segundo", "2"). **No empate, não escolhe** — escolher no empate é responder com confiança sobre o processo errado. E a pergunta passa a ensinar como responder, cuja falta era metade do defeito | Adotar | 🟡 Proposta |
| D-168 | **Pessoas cumprimentam em dobro: "Oi, boa tarde" é uma saudação, não duas.** A regra exigia que a mensagem fosse um cumprimento e mais nada, então a forma mais comum de abrir conversa em português escapava da saudação — e o cliente caía na pergunta sobre qual processo **sem nunca ter sido cumprimentado pelo nome**. Cumprimentos compostos passam a ser reconhecidos; cumprimento **seguido de pergunta** continua sendo pergunta. E quem tem mais de um processo recebe a **lista já na saudação**: sem isso a conversa começa por uma pergunta nossa em vez de uma resposta | Adotar | 🟡 Proposta |
| D-169 | **Voltar a demo ao ar depois que a instância da Uazapi expira é um comando, não uma lista de quatro.** `demo/reapontar.mjs` confere a instância, grava a credencial no n8n, reaponta o webhook e **republica e reativa os dois fluxos**. O passo que mais escapava era o último: sem ele os fluxos seguem citando a credencial antiga, tudo *parece* publicado e o envio só falha na frente de quem está olhando. **Para no primeiro erro** — credencial nova com fluxos velhos é pior que nada feito, porque parece pronto (Regra 5). O token nunca passa pelo script nem pelo chat: sai de `demo/uazapi.local`, gravado por `guardar-segredo.mjs` (D-114, R-51) | Adotar | 🟡 Proposta |
| D-170 | **Papel não informado entra como o de menor privilégio, e isso é decisão, não omissão.** Três IDs de Telegram foram cadastrados para a apresentação sem que o papel fosse dito; entraram como **estagiário, sem poder de aprovar envio ao cliente**. Promover a advogado depois é uma linha de arquivo; **conceder aprovação por engano não se desfaz** — quem aprova faz sair mensagem para cliente de verdade. É a Regra 5 aplicada ao cadastro de pessoas, e não só ao tráfego. Efeito colateral bem-vindo: com convidados como estagiários, a demonstração mostra ao vivo o barramento e o encaminhamento ao advogado | Adotar | 🟡 Proposta |
| D-171 | **Duas colaboradoras aprovam envio ao cliente durante a demonstração — exceção explícita e datada à D-06 e à D-142.** Estefanny e Andressa entram com `pode_aprovar_envio_ao_cliente: true` sem serem advogadas, a pedido do usuário e **só para a apresentação de 02/09**. A configuração não é bloqueada em código de propósito: quem aprova é decisão do escritório, e travar regra de negócio no montador seria a Regra 3 ao contrário. **O risco não é a exceção — é ela sobreviver ao motivo.** Por isso o gerador passa a AVISAR, a cada geração, quem aprova sem ser advogado, com a instrução de reverter. Avisa e não bloqueia: bloquear seria decidir no lugar do escritório; avisar faz a exceção custar uma linha de atenção por vez. **Reverter para `false` ao fim da apresentação** | Adotar, com reversão pendente | 🟡 Proposta |
| D-172 | **O chamado da Demo B vai para todos os aprovadores, não para um.** A primeira versão filtrava por `papel === "advogado"` e mandava para o **primeiro** deles; no teste real o chamado chegou a uma pessoa só, e os colaboradores — que também podem resolver — não ficavam sabendo de nada. Quem é chamado passa a sair de `pode_aprovar_envio_ao_cliente`, e o chamado é espalhado em um item por pessoa. Chamar um só tem o mesmo defeito do encaminhamento antigo: se essa pessoa estiver em audiência, o cliente esperou por quem não vai olhar | Adotar | 🟡 Proposta |
| D-173 | **Avisar quatro pessoas não pode virar quatro respostas ao cliente.** Com vários destinatários, a saída de erro separada do nó do Telegram deixou de servir: uma falha e três sucessos mandariam o fluxo pelos dois caminhos ao mesmo tempo, e o cliente receberia duas respostas contraditórias. Os itens passam a seguir por uma saída só, e um nó reduz o conjunto a uma decisão: **basta UM ter recebido** para que "já avisei o escritório" seja verdade — o escritório foi avisado, não todo o escritório | Adotar | 🟡 Proposta |
| D-174 | **O escritório responde ao cliente pelo próprio chamado, e a resposta é ajustada, não repassada.** O chamado no Telegram ganha o botão "✍️ Responder ao cliente"; a próxima mensagem de quem clicou vira rascunho, o modelo ajusta tom e clareza, e o texto volta como **proposta com os três botões de sempre**. Responder não é atalho: continua exigindo aprovação (Regra 2). Sem o botão, a pessoa lê o chamado, sai do Telegram e responde por outro caminho — e o escritório perde o registro de que respondeu | Adotar | 🟡 Proposta |
| D-175 | **Ao ajustar a resposta, o modelo veste — nunca completa.** É proibido acrescentar o que o rascunho não disse: nem data, nem valor, nem etapa, nem "em breve". Se o rascunho **traz** prazo ou avaliação de chance, mantém-se com as palavras de quem escreveu — é a resposta de uma pessoa habilitada, e é o que o cliente pediu. O que não pode é o modelo preencher a lacuna: **um "deve sair em breve" surgido do nada chega ao cliente com a assinatura do escritório**, e é a alucinação mais cara que este projeto pode produzir | Adotar | 🟡 Proposta |
| D-176 | **As duas demos compartilham o bot, e é isso que fecha o ciclo entre elas.** Só um fluxo pode ter o gatilho do Telegram, então o clique num botão criado pela Demo B chega ao porteiro da Demo A. O ciclo cliente → chamado → resposta → aprovação → cliente atravessa os dois fluxos sem nenhuma ponte extra. O destinatário da resposta é reconferido contra a lista de clientes dentro do fluxo A, e não aceito do botão (Regra 1) | Adotar | 🟡 Proposta |
| D-177 | **O callback e o caminho PRIMARIO para saber de publicacao nova; o polling de `aparicoes` deixa de ser fonte confiavel.** Em 27/08 duas leituras de `/api/v1/monitoramentos/{id}/aparicoes` voltaram `items: []` e o projeto concluiu que a vigilancia nao tinha produzido nada. **Ela tinha:** no mesmo dia, as 06:02 UTC, o Escavador entregou quatro eventos `diario_movimentacao_nova` no receptor, e seguiu entregando todo dia util — 30 ate 02/09. O polling e o callback discordaram, e o polling errou. Enquanto a divergencia nao for explicada — se a rota lista outra coisa, se o parametro estava errado, ou se ha defasagem de indexacao —, o desenho inverte o onus: o callback e o caminho principal, e usar polling para "ha publicacao nova?" e que precisa de justificativa (R-55) | Adotar | 🟡 Proposta |
| D-178 | **Deduplicar publicacao por `movimentacao.id`, nunca pelo `uuid` da entrega.** As 30 amostras trouxeram 30 `uuid` distintos para 30 publicacoes distintas, o que **nao prova nada** sobre reentrega — nenhuma foi reentregue na janela. E o Bloco C ja tinha medido que o `uuid` do Escavador nao serve como chave de idempotencia (`06-orcamento...` §5.6). O `movimentacao.id` e a identidade da publicacao do lado do fornecedor; o `uuid` serve para correlacionar uma entrega no log, e para mais nada | Adotar | 🟡 Proposta |
| D-179 | **`data_disponibilizacao` e `data_publicacao` sao tratadas como campos DISTINTOS, mesmo tendo vindo iguais nas 30 amostras.** A igualdade e observacao sobre dois tribunais em cinco dias, nao contrato. No processo civil os dois conceitos se separam, e a diferenca e justamente onde o prazo mora: publica-se no dia util seguinte ao da disponibilizacao, e o prazo comeca do dia util seguinte ao da publicacao. Tribunal que separe os dois produz evento em que os campos diferem — e tratar a coincidencia de amostra como regra erraria por um dia, que e o erro que perde prazo (R-56) | Adotar | 🟡 Proposta |
| D-180 | **Existe tabela de traducao de tipo de envolvido, e o eixo dela e a FONTE, nao o tribunal — corrige o alcance da D-132.** O callback de diario traz `envolvido_tipo` com `Polo Ativo`, `Polo Passivo` e `Advogado`, e **nao** traz `tipo_normalizado`. Sao tres vocabularios para o mesmo conceito: `RECLAMANTE`/`RECLAMADO` do importador de PDF (D-135), `Autor`/`Reu` da V2 (D-132) e `Polo Ativo`/`Polo Passivo` do diario. A D-132 concluiu que "nao ha tabela de traducao a construir" — o que continua verdadeiro DENTRO da V2, entre ramos da Justica, e e falso ENTRE FONTES. Ha ainda uma assimetria a acomodar: o diario classifica **advogado como um tipo de envolvido** (48 das 100 ocorrencias), enquanto a V2 separa parte de advogado; achatar os dois num campo so perde a distincao | Adotar | 🟡 Proposta |
| D-181 | **O receptor precisa PERSISTIR o evento antes de responder — historico de execucao do n8n nao e armazenamento.** O receptor de hoje tem dois nos, webhook e um `Code` que confere o token e carimba: nao grava em banco, nao enfileira, nao escreve arquivo. O evento existe num lugar so, que e exatamente onde a RNF-08 diz que ele nao pode viver ("fluxo e efemero, prova nao e"). A instancia retem execucoes ha ~302 dias, entao **nao ha perda em curso** — mas retencao e configuracao, nao garantia, e o repositorio ja tem comando para repor a demo depois que a instancia expira | Adotar | 🟡 Proposta |
| D-182 | **A vigilancia `2813617` nao e residuo de teste — e a fonte viva do contrato de prazo.** Ela entrega ~6 publicacoes reais por dia util, de 22 processos, das quais 26 em 30 sao **intimacao**, que e a publicacao que faz prazo correr. As entregas sao gratuitas e continuaram depois de a cota expirar. O que custa e a assinatura, que renova em **26/09**. Remove-la deixou de ser faxina e passou a ser decisao sobre a frente E2 — e, sem saldo, nao ha outra forma de reobter o que ela produz | Decidir com o usuario antes de 26/09 | 🟡 Proposta |
| D-183 | **Teor repetido NAO e duplicata — a restricao `publicacao_hash_unico` cai (migracao 013).** Ela foi desenhada no papel na migracao 006 e so encontrou dado real em 02/09: das 30 publicacoes, entraram 21. Sete PROCESSOS DIFERENTES trouxeram a mesma intimacao padrao de 123 caracteres, e a restricao guardou uma e **descartou seis intimacoes de seis casos distintos** — sem erro, sem contador, sem nada, porque `ON CONFLICT DO NOTHING` nao levanta excecao. Os outros dois grupos eram republicacao do mesmo ato em edicoes seguintes, que e coisa que tribunal faz. A suposicao "duas publicacoes com o mesmo texto sao a mesma publicacao" e plausivel no papel e falsa duas vezes contra o dado. O `hash` continua, como INDICE: passa de restricao que fecha sozinha para pergunta que alguem responde. A mesma suposicao estava em `movimentacao` e caiu junto, enquanto a tabela ainda esta vazia | Adotar | 🟡 Proposta |
| D-184 | **A identidade da publicacao e `id_externo` (o `movimentacao.id` do fornecedor), e a unicidade e por `(inquilino, fonte, id_externo)`.** E o par da D-183: tirada a barreira errada, e preciso dizer qual e a certa. O fornecedor sabe distinguir suas publicacoes melhor do que o nosso resumo de texto — nas 30 amostras os 30 ids foram distintos, **inclusive nas nove que o hash confundiu**. Publicacao sem `id_externo`, como a importada de PDF, fica sem barreira automatica, e isso e o certo: ali nao ha identidade de fornecedor em que confiar, e inventar uma a partir do texto e exatamente o erro que se acabou de desfazer | Adotar | 🟡 Proposta |
| D-185 | **Entrega com origem invalida e GRAVADA, e nao vira publicacao.** Duas gravacoes com valor de verdade diferente: que uma requisicao chegou e fato NOSSO, observado por nos, e vale registrar mesmo quando ela e hostil — entrega nao autenticada e sinal de seguranca, e apagar sinal de seguranca por ele ser invalido e apagar a evidencia de que alguem esta batendo na porta. O que ela DIZ e afirmacao de terceiro, e conteudo externo e hostil (Regra 4). Aceitar afirmacao de origem nao conferida na base de vigilancia seria deixar um desconhecido escrever no acervo que dispara alerta de prazo — e ninguem precisaria invadir nada, bastaria descobrir a URL do webhook | Adotar | 🟡 Proposta |
| D-186 | **Entrega repetida continua tentando gravar o conteudo; "ja processei" e sobre a ENTREGA, nunca sobre tudo o que ela carregava.** A primeira versao devolvia "repetido" e desistia na porta. O efeito so apareceu quando a D-183 foi corrigida: o recolhimento rodou de novo e **nao recuperou nada**, porque os eventos ja estavam gravados. Uma falha no meio do caminho — restricao errada, indisponibilidade, defeito nosso — deixa o evento gravado e o conteudo nao, e se a repeticao desiste, o unico conserto e apagar registro de recepcao para reprocessar. Registro de recepcao nao se apaga. Entao a gravacao da publicacao e idempotente por conta propria e roda sempre: repetir e barato, perder nao e | Adotar | 🟡 Proposta |
| D-187 | **O resultado da gravacao e PLANO, e nao uma uniao de estados.** A primeira versao era `gravado | repetido | recusado`, e ela mentia: "o evento ja estava na base" e "a publicacao entrou agora" sao fatos INDEPENDENTES, e no recolhimento de 02/09 nove publicacoes foram recuperadas de eventos gravados havia minutos. A uniao obrigou a escolher um rotulo, o codigo escolheu `gravado`, e o relatorio disse "9 eventos novos" sobre nove que nao tinham nada de novos. Tipo que forca a escolher entre dois fatos verdadeiros produz relatorio falso — e relatorio de recolhimento e o que alguem le para decidir se investiga | Adotar | 🟡 Proposta |
| D-188 | **A idempotencia de callback passa a ser por escritorio (migracao 012).** `evento_callback_chave_unica` era `(fornecedor, chave_evento)`, global — e a chave e resumo de conteudo, entao dois escritorios em polos opostos da mesma acao produzem a mesma chave. Com a politica por linha da migracao 010 ligada, o segundo INSERT colide com uma linha que a conexao **nao consegue enxergar**: ou falha por politica, ou conclui "ja processei" sobre evento que aquele escritorio nunca recebeu. O segundo caso nao levanta erro nenhum — a publicacao simplesmente nao entra, e ninguem descobre ate faltar um prazo. Fecha o achado 13 da revisao de 28/08. Corrigido enquanto e inerte, porque o dia em que deixar de ser e o dia da migracao de um cliente novo | Adotar | 🟡 Proposta |
| D-189 | **A lista do que conta como trabalho vira lista de EXCLUSAO, no fechar-ciclo.mjs.** A lista de inclusao — docs/, demo/, captura/, .claude/ — foi escrita em 25/08, quando o projeto ainda era so documentos, e nunca foi atualizada. Quando o codigo chegou, services/, dados/, testes/ e ferramentas/ nasceram invisiveis: medido em 04/09, o hook nao enxergava **13 dos 24 arquivos** dos ultimos dez commits, e uma sessao inteira construindo o receptor de callbacks parava em silencio com o estado envelhecendo. O defeito nao foi a lista estar errada: foi a FORMA da lista. Com inclusao, cada pasta nova nasce cega ate alguem lembrar de cadastra-la — e ninguem lembrou por dez dias. Com exclusao, a pasta nova ja nasce coberta, e o esquecimento passa a errar para o lado seguro | Adotar | 🟡 Proposta |
| D-190 | **A autoria de uma alteracao vem do que a sessao PROVADAMENTE escreveu, nunca da diferenca no disco.** O fechar-ciclo comparava o `git status` do fim com a fotografia do inicio e atribuia a esta sessao tudo que mudasse no meio. Num projeto tocado por varias sessoes em paralelo a premissa e falsa, e em 02/09 ela barrou uma sessao que so LEU arquivos, cobrando-a por alteracoes que outra sessao fizera segundos antes. A sessao inocente fica com duas saidas ruins: inventar uma atualizacao de estado sobre trabalho alheio, ou aprender a ignorar o hook — e a segunda e a que acontece. O conserto e parar de adivinhar: o `anotar-escrita.mjs` (PostToolUse) registra cada arquivo escrito enquanto acontece. Onde a prova nao alcanca — arquivo escrito pelo terminal, que nao diz o que tocou — o hook cobra assim mesmo, mas **declara que a autoria nao esta provada**, porque incerteza fecha (Regra 5) e barreira que mente sobre o que sabe e pior que barreira que admite o limite | Adotar | 🟡 Proposta |
| D-191 | **Comando que o usuario roda vira atalho do `npm run`, e nunca depende da sintaxe de um shell.** A instrucao entregue em 02/09 era `LEX_INQUILINO_ID=... node ...`, sintaxe de **bash**. A maquina de trabalho e Windows: o PowerShell le o texto inteiro como nome de programa e responde *"nao e reconhecido como cmdlet"*. O comando estava certo e mesmo assim nao funcionava, o que e a pior categoria de erro — quem recebe nao tem como distinguir "escreveram errado" de "eu digitei errado". **A licao nao e documentar as duas sintaxes, e nao depender de nenhuma:** `npm run` e o mesmo texto em bash, PowerShell e cmd. O parametro que faltava passou a vir, em ordem, de `--inquilino <uuid>`, do ambiente, ou de `infra/.env` — e este ultimo e o normal, porque configuracao que se digita toda vez e configuracao que um dia se digita errado | Adotar, e vale para todo comando que apareca em documento ou em resposta ao usuario | 🟡 Proposta |
| D-192 | **O colaborador enxerga a base inteira, exatamente como o advogado — e o perfil continua individual.** Respondida em 05/09: o escritorio confirmou que colaborador tem acesso completo, porque *"eles fazem movimentacao em nome dos advogados e com o acesso e credenciais dos advogados, sob observacao dos advogados"*. O padrao restritivo provisorio da pergunta 4a **cai**. O que **nao** cai e a identidade individual (Regra 7): o usuario registrou que a individualizacao de perfis fica mantida para as sete pessoas, para que toda acao seja registrada e auditavel. A leitura correta e que **alcance e permissao sao eixos separados**: dar a todos a mesma visao nao autoriza a todos os mesmos atos, e nao dispensa saber quem fez o que. O escopo de LEITURA passa a ser o escritorio inteiro para os dois papeis; o escopo de ACAO continua vindo do papel | Adotar — atualizar a matriz de privilegios da Spec Parte II | 🟡 Proposta |
| D-193 | **A carteira e de centenas, nao de milhares — ~313 processos ativos, sem cliente de alto volume, e P-07 se confirma.** Medido em 05/09 pelo escritorio: ~289 processos da advogada Malu Souza e ~24 da Ana Beatriz, e **nenhum cliente concentra 200 processos ou mais**. As duas contagens que faltavam para P-07 chegaram, e a premissa se sustenta: consulta ampla cabe em um bloco de 200 na maioria das vezes, e o pior caso conhecido e **dois blocos** para varrer a carteira inteira — nao dez. R-25 fica pequeno, e os tetos da PRD §9.2 nao precisam ser afrouxados. **Um efeito colateral pesa mais que o custo:** com sete pessoas e sem hierarquia (perguntas 1 e 2), a escalada de prazo **nao tem terceiro nivel** — o "N2 para a socia responsavel" da pergunta 20b nao existe, e o rito precisa terminar nas duas advogadas | Adotar; fechar P-07 no PRD §11.1 e redesenhar o N2 do rito de prazo | 🟡 Proposta |
| D-194 | **Proposta: colaborador aprova mensagem informativa ao cliente; ato com efeito de prazo exige advogado identificado.** A pergunta 6 voltou em 05/09 com *"todos aprovam, sob observacao, orientacao e acompanhamento dos advogados"* — o que descreve com honestidade uma sala de sete pessoas, e nao sobrevive a um botao. Supervisao presencial e continua e informal; aprovacao em sistema e pontual e registrada, e **ou exige advogado, ou nao exige**. A Regra Inegociavel 2 nao admite o meio-termo. A proposta separa por CONSEQUENCIA, nao por assunto nem por valor: mensagem que informa (andamento, agenda, confirmacao de recebimento) qualquer perfil aprova; ato que produz efeito juridico ou faz prazo correr so avanca com aprovacao de advogada identificada. Isso **nao tira trabalho do colaborador** — ele continua preparando o ato inteiro; muda apenas quem carimba o ultimo passo | **Aguarda o "de acordo" do escritorio.** Se recusado, e conflito com regra inegociavel e sobe como decisao formal, nao como configuracao | 🔴 Em aberto |
| D-195 | **O prazo e 10/09 como ideal e 15/09 como limite, e ele nao cabe com as quatro frentes dentro — a decisao e QUAL escopo entra, e ela e agora.** Respondido em 05/09, com contratacao em escopo fechado (pergunta 73), o que significa que estouro nao vira hora extra: vira escopo nao entregue. Contra o calendario: o saldo do fornecedor processual so e contratado em **08/09**, a chave do Trello ainda nao existe, e **WhatsApp nao tem conta, numero nem template aprovado**. E1 e E2 sao viaveis — a fundacao, o chassi e a auditoria ja estao de pe e verificados, e a vigilancia ja entrega publicacao real por callback desde 27/08 sem custo. **E4 nao e**, por motivo que nao esta ao nosso alcance apressar (D-197). E3 depende de uma chave que chega quando chegar. A proposta e entregar **E1 + E2 em producao ate 15/09**, com E3 e E4 como fase seguinte e data propria | **Aguarda o de acordo do usuario e da Malu Souza.** Escopo fechado sem recorte acordado antes do inicio e o modo classico de a entrega falhar no dia 14 | 🔴 Em aberto |
| D-196 | **A chave do Trello nasce numa conta dedicada do escritorio, nao na conta da advogada chefe.** Respondida a pergunta 66 em 05/09: "conta de servico" nao existe como recurso no Trello — o que existe e uma conta de pessoa comum criada para uso exclusivo da automacao, com e-mail proprio do escritorio, convidada para os quadros. Tres motivos, e o primeiro e do projeto inteiro: (1) com a conta da advogada, **toda acao da plataforma aparece no quadro como feita por ela**, e a auditoria que sustenta o projeto vira ficcao dentro do Trello; (2) a chave de API dela alcanca tudo o que ela alcanca, inclusive quadro pessoal fora do escritorio; (3) troca de senha, revogacao de token ou desligamento param a plataforma, e ninguem liga uma coisa a outra. Custa um e-mail e uma licenca | Adotar, **antes** de gerar a chave | 🟡 Proposta |
| D-197 | **Recomendacao: WhatsApp por BSP, e nao direto pela Meta — e o motivo e o calendario, nao a tecnologia.** O usuario preferiu a Meta em 05/09, e a preferencia e defensavel: mais barato por mensagem, sem intermediario. **O que a derruba e o prazo da D-195.** Ir direto exige Business Manager, verificacao de negocio (documento de CNPJ, e o tempo de analise e da Meta), numero dedicado que **nao pode estar em uso no WhatsApp comum**, e aprovacao de cada template. Semanas, e nenhuma delas sob nosso controle. Um BSP chega verificado: numero e template no mesmo dia, mais caro por mensagem. Com limite em 15/09, "mais caro por mensagem" e irrelevante e "semanas" e fatal. **A alternativa honesta e a D-195:** se E4 sair do escopo desta fase, a Meta volta a ser a escolha certa, e o processo dela comeca em paralelo sem apertar ninguem | **Decidir junto com D-195** — as duas sao a mesma decisao vista de dois lados | 🔴 Em aberto |
| D-198 | **O provedor de modelos passa a ser o OpenRouter, e isso e decisao de arquitetura, nao de compras.** Informado em 05/09: OpenAI e Gemini ja existem, e o OpenRouter sera contratado para alcancar todos os modelos, inclusive o Claude. O ganho nao e o catalogo: e que a plataforma passa a ter **um** provedor na configuracao, e **trocar de modelo deixa de ser trocar de credencial**. Com escolha de modelo virando parametro, o custo por tarefa vira ajuste em vez de reimplantacao — o que casa com a Regra 6 (custo e requisito funcional). O preco disso e uma dependencia nova no caminho de toda chamada de IA, e um intermediario a mais vendo o conteudo dos prompts: **prompt com dado de processo passa a transitar por um terceiro**, o que entra no rol de operadores a declarar (RNF-19) | Adotar; registrar o OpenRouter entre os operadores de dados | 🟡 Proposta |
| D-199 | **O codigo e do prestador, licenciado ao escritorio — e isso promove a Regra 3 de higiene de arquitetura a clausula de contrato.** Respondida a pergunta 74 em 05/09. A Regra Inegociavel 3 ja mandava que regra de negocio do escritorio ficasse fora dos servidores MCP, para que eles fossem genericos e reutilizaveis; ate aqui isso era disciplina nossa, que so a nos custava quebrar. Com a propriedade definida, passa a ser o que torna o licenciamento possivel: **MCP contaminado com regra do escritorio e MCP que nao pode ser licenciado a mais ninguem**. O contrato precisa dizer as duas coisas — que a plataforma e licenciada, e que os dados do escritorio sao do escritorio | Adotar; refletir no contrato e manter o Policy Gate como o lugar da regra de negocio | 🟡 Proposta |
| D-200 | **A responsavel interna pelo projeto e a advogada Malu Souza.** Respondida a pergunta 5 em 05/09, depois de o escritorio pedir que o papel fosse explicado. E o interlocutor unico de **negocio**: da o "de acordo" nos numeros propostos, decide quando dois pedidos do escritorio se contradizem, e responde a descoberta. Distinto do **administrador da plataforma** (quem cadastra e desliga contas) — que pode ser a mesma pessoa, mas executa em vez de decidir. Coerente com a carteira: dos ~313 processos, ~289 sao dela (D-193). **Consequencia pratica imediata:** as decisoes hoje em aberto que dependem do escritorio — D-194, os numeros do PRD §9, o rito de prazo 20a-20d e o recorte de escopo da D-195 — tem agora um destinatario com nome | Adotar | 🟡 Proposta |
| D-201 | **O banco da plataforma nao compartilha instancia nem credencial com o n8n.** Medido em 05/09: o n8n conecta ao PostgreSQL como **`postgres`, o superusuario da instancia** (`DB_POSTGRESDB_USER=postgres`). Quem tem essa credencial alcanca todos os bancos daquele servidor, nao so o `n8n_database` — e ela vive em variavel de ambiente lida por qualquer um que entre no Portainer, que esta publico (R-62, R-64). Se a base de governanca, auditoria e vigilancia morasse ali, a politica por linha da migracao 010 e a auditoria imutavel do marco 3 seriam **contornaveis por fora**, sem tocar em nada que o nosso codigo vigia: bastaria abrir o banco com a credencial do n8n. **Barreira que se contorna pela porta dos fundos nao e barreira.** A plataforma ganha instancia propria, ou no minimo papel proprio sem superusuario e banco proprio, com credencial que nenhum fluxo de terceiro conhece | Adotar; e pre-requisito do marco de implantacao | 🟡 Proposta |
| D-202 | **A infraestrutura de WhatsApp NAO oficial ja existe no servidor, e isso vira decisao do usuario e da Malu — nao um descarte nosso.** Medido em 05/09: `evolution-api v2.3.1`, `wuzapi` e `chatwoot v4.1.0` estao no ar, com endereco publico. Ate aqui, D-10 e P-05 recusaram biblioteca nao oficial por risco de **banimento do numero**, e a recusa custava pouco porque a alternativa era abstrata. Agora nao e: a Evolution sobe um numero hoje, sem verificacao de negocio na Meta e sem template aprovado, e **e a unica opcao de WhatsApp que cabe no prazo da D-195**. O trade-off e nitido e nao e tecnico: pelo caminho oficial, E4 nao entrega ate 15/09; pelo nao oficial, entrega e o escritorio corre o risco de perder o numero pelo qual seus clientes falam com ele — de um dia para o outro, sem aviso e sem recurso. **Minha posicao continua sendo tirar E4 desta fase** (D-195) em vez de escolher entre os dois. Mas a escolha nao e minha, e escondê-la seria pior que faze-la | **Decidir junto com D-195 e D-197.** Se o nao oficial for escolhido, tem que ser com o risco escrito e aceito por escrito | 🔴 Em aberto |
| D-203 | **A retencao de execucao e de 14 dias OU 10.000 execucoes, o que vier primeiro — e o R-57 deixa de ser estimativa.** Lido na configuracao em 05/09: `EXECUTIONS_DATA_MAX_AGE=336` horas, `EXECUTIONS_DATA_PRUNE_MAX_COUNT=10000`, poda definitiva a cada 15 minutos. Em 02/09 o projeto observou 11.076 execucoes desaparecidas de 11.249 e concluiu, corretamente, que a primeira leitura ("~302 dias de retencao") estava errada por ter olhado a mais antiga **sobrevivente**. Agora o numero esta configurado, e num servidor com fluxos comerciais ativos **o teto de 10.000 chega bem antes dos 14 dias**. Consequencia: entrega de callback que fique so no historico de execucao tem prazo de validade curto e **variavel com o movimento de outros clientes** — o que e pior que curto, porque nao da para planejar em cima. O receptor gravar na hora (D-181, marco 7) deixa de ser boa pratica e passa a ter numero em cima | Adotar; o marco 7 sobe de prioridade | 🟡 Proposta |
| D-204 | **O teto de 16 MB por requisicao no n8n e restricao de projeto para E3, e o desvio e o MinIO.** `N8N_PAYLOAD_SIZE_MAX=16` foi lido em 05/09. Autos em PDF passam de 16 MB com frequencia — e o importador de autos ja construido trabalha exatamente com esse tipo de arquivo. Trafegar o PDF inteiro por dentro do fluxo falha de um jeito ruim: nao e erro de logica, e a requisicao ser recusada na porta, o que aparece como "nao chegou" e nao como "era grande demais". O caminho e o arquivo ir para o **MinIO** (armazenamento de objetos ja no ar no servidor, com endereco `s3.criativeia.com.br`) e o fluxo carregar so a referencia. Como efeito colateral bom, isso tira o documento do historico de execucao, que e podado (D-203) | Adotar no desenho de E3 | 🟡 Proposta |

> D-16 a D-21 são fundamentadas na [Nota Técnica 01](03-canais-internos-e-hospedagem.md); D-22 a D-26, no [Modelo de Identidade e Autorização](04-modelo-de-identidade-e-autorizacao.md); D-27 a D-35, no [Mapeamento da API do Escavador](mapeamento-escavador.md); D-36 a D-46, no [Mapeamento da API do Trello](mapeamento-trello.md); D-47 a D-50, no [Orçamento de Chamadas do Escavador](06-orcamento-de-chamadas-escavador.md); D-51 a D-60, nos [Achados do Painel do Escavador](07-painel-escavador-achados.md); D-61 a D-67, no [PRD](08-prd.md); D-68 a D-78, na [Spec Técnica — Parte I](09-spec-tecnica.md); D-79 a D-85, na [Nota Técnica 02 — ClickUp](10-clickup-avaliacao.md); D-86 a D-94, na [Nota Técnica 03 — Demonstração](11-nota-tecnica-demo.md); D-95 e D-96, no [Orçamento de Chamadas](06-orcamento-de-chamadas-escavador.md); D-97 e D-98, no [Contrato do Instantâneo](../demo/CONTRATO-DO-INSTANTANEO.md); D-99 e D-100 decorrem da Regra 2 e da D-91; D-99 a D-102 estão implementadas em `demo/montar-fluxo-a.mjs` e `demo/montar-fluxo-b.mjs`, com regressão em `demo/testar-fluxo-*.mjs` (aprovar no Telegram dispara o envio no WhatsApp); **D-142 a D-152 nascem das respostas do escritório de 27/08**, registradas no [PRD](08-prd.md) §15; **D-155 e D-156 nascem da revisão externa de 28–31/08**: a D-155 corrige a Demo B e a D-156 leva a D-142 para dentro do `mcp-core`, com regressão em `demo/testar-fluxo-b.mjs` e `testes/chassi.test.mjs`; **D-157 a D-162 nascem do [Marco 3 — Auditoria](14-auditoria-marco-3.md)**, provadas por `npm run banco:auditoria` (23 de 23 contra um PostgreSQL de pé). **D-177 a D-182 nascem do [Contrato da Aparição](15-contrato-da-aparicao.md)**, medido em 30 entregas reais de callback entre 27/08 e 02/09, a custo zero.
...
| R-50 | **Sessao que nao esta no banco torna toda chamada dela impossivel.** `evento_auditoria.sessao_id` tem chave estrangeira para `sessao`. Uma sessao emitida pelo Policy Gate e mantida so em memoria nao pode ser registrada — e, pela D-77, o que nao se registra nao acontece | **Grave pelo sintoma, nao pela causa.** O acoplamento esta certo: trilha que aponta para uma sessao que ninguem descreve nao reconstroi nada. O problema e o efeito em cascata — o sintoma nao e "a auditoria falhou", e **"a plataforma inteira parou"**, e nada no erro aponta para a sessao | O Policy Gate do marco 9 **persiste a sessao na emissao**, antes de devolver o token. Descoberto ao integrar o marco 3 e registrado aqui porque nao estava escrito em lugar nenhum; ate la, quem montar cenario de teste cria a linha de `sessao` (ver `ferramentas/banco/conferir-auditoria.mjs`) |
| R-51 | **O painel do fornecedor exibe o token da instância em tela, e ele vaza por captura.** A D-114 tirou o segredo da saída dos nossos scripts — mas o painel da Uazapi mostra o token em texto aberto, e a captura de tela o carrega para fora | **Moderado, e fora do nosso alcance.** A disciplina de segredo que escrevemos no código não alcança a interface do fornecedor. Em produção o campo equivalente **dá acesso a mandar mensagem como o escritório**; quem tem o token é o escritório, para quem receber | Segredo se lê pelo script, nunca pelo painel; se o painel for inevitável, tape o campo antes da captura. Na plataforma real, token de canal externo mora no cofre e nenhuma tela o exibe. Ocorrência real em 01/09/2026, sem dano — instância gratuita de 1 hora |
| R-52 | **O chamado ao advogado pode virar ruído, e chamado que ninguém lê é pior que chamado nenhum.** Três gatilhos numa demonstração com um cliente é confortável; num escritório com centenas, "perguntou sobre prazo" chega dezenas de vezes por dia, e o mecanismo morre de sucesso — sem que ninguém desligue nada, apenas por deixarem de olhar | **Novo, moderado, e é o risco que a própria D-163 cria.** O sintoma não é erro: é o chamado continuar sendo enviado e ninguém responder, o que faz o cliente ouvir "já avisei o escritório" e não receber contato. Aí a frase volta a ser falsa — não por defeito de código, mas por saturação | Na plataforma real: fila por advogado responsável em vez de destinatário único, agrupamento por cliente, e **medição de tempo até a primeira resposta humana** como métrica de operação. Na demo, um destinatário só é aceitável e deliberado |
| R-53 | **Defeito de conversa não dá erro, não vaza nada e passa por toda a bateria de testes.** Os dois achados do primeiro uso real — o laço da escolha de processo e a saudação composta — atravessaram 86 verificações automáticas sem acender uma luz. Nenhum é falha de segurança: em ambos o sistema faz exatamente o que foi mandado, e é a conversa que fica impossível | **Novo, moderado, e estrutural na forma como testamos.** A suíte verifica o que o código decide, não o que a pessoa consegue fazer com ele. Uma conversa que não anda tem custo real — na demonstração, para o escritório; em produção, para o cliente que desiste | Toda cena da demonstração passa a ter uma **transcrição de conversa** entre as verificações, não só asserções sobre campos; e o primeiro uso real de qualquer caminho novo é tratado como parte do teste, não como confirmação dele |
| R-54 | **Teste que deriva da configuração pode perder cobertura quando a configuração cresce, e o sintoma é uma reprovação que parece defeito do fluxo.** Ao passar de um para três clientes, o fluxo A reprovou três verificações: o teste escolhia o "processo sem destinatário" excluindo apenas os processos do **primeiro** cliente, e acabava pegando um que passara a ter dono. Pior: com todo processo fora do segredo vinculado a alguém, **deixa de existir processo real que exercite aquele caminho** | **Novo, baixo, e já corrigido** — mas a forma dele reaparece a cada mudança de lista. Derivar da configuração é o que mantém a suíte honesta; o preço é que a suíte precisa cobrir também as formas que a configuração pode assumir | O teste passa a excluir os processos de **todos** os clientes e, quando não sobra nenhum real, usa um id que não existe em lista alguma — o caminho continua exercitado independentemente de como a lista esteja. Mesma disciplina já aplicada em "cliente com um processo" vs "com vários" |
| R-55 | **O polling de `aparicoes` e o callback discordaram, e o polling e que estava errado.** Duas leituras da rota em 27/08 voltaram vazias enquanto o callback entregava quatro eventos no mesmo dia. Nao se sabe por que: se a rota lista outra coisa, se a chamada usou parametro errado, ou se ha defasagem de indexacao | **Grave, e silencioso.** O sintoma de uma fonte de publicacao que responde "nada novo" quando ha algo novo e indistinguivel de um dia tranquilo — e o custo de errar e prazo perdido, que o proprio PRD chama de pior cenario do projeto | O callback vira o caminho primario (D-177). A rota de leitura so volta a ser usada para essa pergunta depois que a divergencia for explicada **por medicao**, o que exige saldo. Ate la, conferencia humana continua sendo a rede |
| R-56 | **`data_disponibilizacao` e `data_publicacao` vieram iguais nas 30 amostras, e nao ha garantia de que continuem.** Dois tribunais, cinco dias | **Grave se virar suposicao.** Os dois conceitos se separam no processo civil, e a diferenca e de um dia util — exatamente a margem que decide se um prazo foi cumprido | Tratar os campos como distintos em codigo e em esquema (D-179); nunca derivar um do outro; e, quando divergirem pela primeira vez, registrar a amostra — sera o primeiro dado real sobre o comportamento do tribunal que separou |
| R-57 | **A instancia do n8n descarta execucoes, e o receptor so gravava ali.** Medido em 02/09: ids de execucao de 492 a 11.740 e **173 sobreviventes** — 11.076 sumiram, 1,5% de sobrevivencia. O que sobra parece ser o que alguem anotou a mao no historico | **Grave, e foi subestimado uma vez.** A primeira leitura olhou a data da execucao mais antiga SOBREVIVENTE (04/11/2025) e concluiu "~302 dias de retencao, nao ha perda em curso". Sobrevivente nao e amostra da populacao — o erro de raciocinio quase deixou o contrato mais importante do projeto expirar numa fila de poda | As 35 entregas foram recolhidas para o banco em 02/09 (`ferramentas/receptor/recolher-do-n8n.mjs`), com o payload em `captura/respostas-brutas/callbacks/`. Enquanto o recolhedor for o unico caminho, **entrega que chegue e seja podada antes do proximo recolhimento some** — o conserto definitivo e o receptor gravar na hora, e depende de endpoint publico da plataforma (marco 7). ✅ **05/09: a retencao deixou de ser inferida.** A configuracao diz `EXECUTIONS_DATA_MAX_AGE=336` horas (14 dias) e `PRUNE_MAX_COUNT=10000`, o que vier primeiro, com poda a cada 15 minutos — e num servidor com fluxos comerciais ativos o teto de 10.000 chega antes. A janela e curta E **variavel com o movimento de outros clientes**, o que e pior que curta. Ver D-203 |
| R-58 | **Barreira que da alarme falso nao e um incomodo: e o que ensina a proxima sessao a desliga-la.** Ja mordeu duas vezes. Em 27/08 o `guarda-segredo` bloqueou `const senha = env.LEX_APP_PASSWORD`, que e o codigo fazendo exatamente o que o guarda quer. Em 02/09 o `fechar-ciclo` cobrou uma sessao de leitura pelo trabalho de outra sessao | **Estrutural, e cresce com o numero de barreiras.** O projeto tem cinco hooks e vai ganhar mais. Barreira desligada nao avisa que foi desligada — o custo aparece so no dia em que ela teria pego algo de verdade | Todo hook nasce com suite de testes propria, e os casos que precisam PASSAR pesam tanto quanto os que precisam bloquear (o `testar-anotar-escrita` tem 9 casos de "nao dispare" para 7 de "dispare"). Quando um falso positivo aparece, ele vira caso de teste com a data e o motivo, no codigo — foi o que se fez com `env.LEX_APP_PASSWORD` e com `2>/dev/null` |
| R-59 | **A auditoria da plataforma nao alcanca o que e feito fora dela, e hoje o que e feito fora dela usa a credencial de outra pessoa.** A resposta a pergunta 4a (05/09) descreve a pratica atual: colaboradoras atuam nos sistemas externos — tribunal, PJe, e-mail — **com as credenciais das advogadas**. A plataforma vai registrar com precisao quem pediu o que dentro dela, e nao tem como registrar nada do lado de fora | **Novo, moderado, e fora do nosso alcance direto.** O efeito e uma trilha que parece completa e nao e: um ato peticionado no tribunal aparece assinado pela advogada, sem dizer quem operou o teclado. Se um dia for preciso reconstruir o que aconteceu, a parte de dentro responde e a de fora nao. E parente de R-11, e sobrevive a ele: R-11 se resolve com licenca individual do Workspace; este so se resolve com credencial individual **no tribunal**, que nao esta ao nosso alcance | Nao prometer o que nao se pode cumprir: a auditoria da plataforma se declara como registro **dos atos da plataforma**, e nunca como registro da atuacao do escritorio. Onde a plataforma for a origem do ato externo — peticionamento, envio ao cliente —, ela grava o solicitante individual (D-192), e essa e a parcela que ganhamos. Levar ao escritorio como observacao, nao como exigencia: e decisao interna deles, e o usuario registrou que nao tem ingerencia sobre ela |
| R-60 | **A instancia do n8n e compartilhada com os outros clientes do prestador, e `/metrics` conta isso a quem perguntar.** Medido em 05/09: 194 fluxos, 9 ativos, e **cinco dos nove sao de outros clientes**. O endpoint `/metrics` responde **sem autenticacao nenhuma** e entrega a versao exata (`v1.123.18`), a contagem de fluxos ativos, memoria e tempo no ar | **Novo, e de duas naturezas.** A versao exata e o primeiro dado que alguem procura para escolher uma falha conhecida a tentar — e uma instancia no ar ha 43 dias sem reinicio nao recebeu atualizacao nesse periodo. O compartilhamento e o problema maior e mais lento: **dado de processo de um escritorio de advocacia passa a residir na mesma instancia que a automacao comercial de outros clientes**, e a D-148 (a infra e do prestador) fez o escritorio aceitar isso sem que o numero estivesse na mesa | Fechar `/metrics` a quem nao for o coletor — e a correcao barata, e nao depende de nada. Sobre o compartilhamento, a decisao e do usuario e tem custo: instancia dedicada para o escritorio resolve de vez; enquanto nao houver, o dado do escritorio vive em banco proprio da aplicacao (nunca no banco do n8n), e o n8n so ve o que precisa passar por ele. Ver `16-levantamento-instancia-n8n.md` |
| R-61 | **Existe uma credencial da API do proprio n8n dentro do n8n, e ela nao tem escopo.** Medido em 05/09: a credencial `n8nApi` "Conexao Geral" e usada por **dois fluxos de producao de outros clientes** (Font Liberty e Nexus AI), em nos que apagam execucoes em andamento. Chave de API do n8n **nao tem escopo fora do Enterprise** (R-38): alcanca os 194 fluxos, as credenciais e todo o historico de execucao — inclusive os deste projeto | **Novo, grave, e nao e sobre ma-fe.** E sobre alcance: um defeito, uma expressao mal escrita ou um prompt manipulado num fluxo comercial de outro cliente tem, tecnicamente, o alcance de apagar execucao deste projeto — e execucao e onde a entrega de callback vive ate ser recolhida (R-57). Os dois fluxos que a carregam sao acionados por **webhook sem autenticacao no no**, e o que os protege, se algo os protege, nao foi auditado por nos | **Nao mexer nos fluxos dos outros clientes** — nao sao nossos. O que esta ao nosso alcance: a persistencia deste projeto nao pode depender do historico de execucao do n8n (ja e a D-181 e o marco 7), e o banco da aplicacao fica **fora** do n8n, com credencial que nenhum fluxo de terceiro conhece. Levar ao usuario como decisao de infraestrutura, junto de R-60 |
| R-62 | **O plano de controle da infraestrutura esta publicado na internet.** Medido em 05/09: treze servicos respondem publicamente pelo Traefik, e tres deles nao deveriam. **Portainer** em `container.criativeia.com.br` — quem entra manda nas 22 stacks, le todos os segredos, sobe e derruba qualquer coisa. **Prometheus** em `dashprometheus...` — e Prometheus **nao tem autenticacao por padrao**, entao provavelmente responde a qualquer um agora, carregando ainda `N8N_METRICS_INCLUDE_WORKFLOW_ID_LABEL=true`, que vaza id de workflow. **pgbackweb** em `pgbackweb...` — interface de backup e restauracao de banco. Grafana, RabbitMQ e MinIO tambem estao publicos, com login, mas a senha inicial nao foi conferida | **Novo, e o mais grave do levantamento.** Nao e ataque nem descuido deliberado: e o resultado de subir stacks prontas de catalogo, cada uma com o rotulo do Traefik ja preenchido — ninguem *decidiu* expor o Portainer, ele veio exposto. E comum. Mas sob a D-148 a infra e do prestador, o que o torna **operador de dados** perante a LGPD, e dado de escritorio de advocacia vai morar ai | Tirar Portainer e Prometheus da internet, ou fecha-los atras de autenticacao no Traefik — Portainer nao precisa de endereco publico. Conferir senha inicial de Grafana, RabbitMQ e MinIO. Nada disso trava o projeto, e tudo isso precede dado real do escritorio. ⚠️ **05/09: o enxame tem DOIS nos, nao um.** O `pgbackweb` esta agendado em `srv1093898`, e o usuario estava conectado em `srv957606` — e o `portainer_agent` roda em modo `global 2/2`, que confirma. **Existe uma segunda maquina que este levantamento nao olhou**, e R-62 a R-66 podem valer para ela tambem |
| R-63 | **CONFIRMADO em 05/09: o PostgreSQL responde à internet na porta 5432.** `Test-NetConnection` de fora da rede do servidor devolveu **`TcpTestSucceeded: True`**. `pgbouncer` publica `*:5432`, `postgresbackupmcp` publica `*:5433` e `wootrico` publica `*:3000` — o `*:` e todas as interfaces, o IP publico incluido. A suspeita era que em Swarm a porta publicada atravessa o `ufw` pela forma como o Docker escreve as regras de rede. **Atravessa mesmo** | **CONFIRMADO, e e o achado mais urgente do levantamento.** O que isto significa com precisao: **a porta esta alcancavel**, e a unica coisa entre a internet e o banco e a senha. Qualquer pessoa pode tentar senha indefinidamente, sem limite de tentativa e sem ninguem ver, e qualquer falha conhecida do PostgreSQL ou do pgbouncer fica ao alcance direto. **Nao significa que alguem entrou** — nao ha evidencia em nenhuma direcao, e conferir os registros de autenticacao do banco e o proximo passo. Este e o banco que guarda os 194 fluxos, e e onde a plataforma NAO vai morar (D-201) | ⚠️ **Fechar exige cuidado, porque pode quebrar o acesso legitimo do proprio usuario** — porta de banco publicada costuma existir porque alguem se conecta de fora com cliente de banco. A correcao certa nao e derrubar a porta as cegas: e o **firewall do painel da Hostinger**, que fica ACIMA do host e por isso nao e atravessado pelas regras do Docker, liberando so o IP de quem precisa. Alternativa melhor a medio prazo: tirar a publicacao da stack e acessar por tunel SSH. Decisao do usuario, com o custo de cada caminho na mesa |
| R-64 | **Os segredos vivem em variavel de ambiente, em texto plano, e o Swarm tem cofre nativo que nao esta em uso.** `DB_POSTGRESDB_PASSWORD`, `N8N_ENCRYPTION_KEY` e `N8N_SMTP_PASS` estao no ambiente do servico. O `docker secret` existe e nao foi usado | **Novo, e e o R-62 visto de outro lado.** Quem le o ambiente de um servico le os segredos; quem entra no Portainer le o ambiente de **todos**. Isoladamente seria aceitavel — infra pequena, um administrador. Junto com o plano de controle publicado, deixa de ser | Responde a pergunta 54 da descoberta. A correcao de verdade e o cofre; a correcao barata e fechar o Portainer (R-62), que ja tira o alcance. Para a plataforma, o segredo do banco proprio (D-201) nasce fora deste arranjo |
| R-65 | **O no de codigo do n8n alcanca o sistema de arquivos e pode abrir processo, em qualquer um dos 194 fluxos.** `NODE_FUNCTION_ALLOW_BUILTIN=*` permite carregar qualquer modulo interno do Node — `fs`, `child_process`, `net`. Somado a `N8N_COMMUNITY_PACKAGES_ENABLED=true` e `N8N_REINSTALL_MISSING_PACKAGES=true`, que instalam pacote de terceiro e o reinstalam sozinhos quando falta | **Novo, moderado, e sutil.** A instancia acertou em ligar `N8N_BLOCK_ENV_ACCESS_IN_NODE`, que impede o no de codigo de ler variavel de ambiente — mas esse bloqueio vale menos quando se pode abrir um processo e chegar ao ambiente por outro caminho. Os dois ajustes puxam para lados contrarios. E o alcance nao e so nosso: vale para os fluxos dos outros clientes, que compartilham a instancia (R-60) | Fora do nosso alcance corrigir sem afetar os outros clientes — restringir `ALLOW_BUILTIN` pode quebrar fluxo de terceiro que dependa disso. Levar ao usuario como decisao de infraestrutura. Para a plataforma, reforca a D-201: o que o no de codigo alcanca nao pode incluir o banco de governanca |
| R-66 | **A ferramenta de backup do PostgreSQL esta instalada, publicada, e PARADA.** `pgbackweb_pgbackweb` roda com **0 de 1 replica**. **Medido em 05/09: nao esta "parado", esta em laco de falha** — reinicia e morre com `non-zero exit (201)` a cada ~6 segundos, indefinidamente. Ou seja, o Swarm vem tentando subi-lo sem parar, e nenhum backup roda ha tanto tempo quanto isso dura | **Novo, grave, e fecha um ciclo ruim.** A pergunta 46 achou **194 fluxos existindo em um lugar so, sem versionamento**; esse lugar e este PostgreSQL; e o backup dele esta fora do ar. Um salvamento por cima, uma migracao malsucedida ou uma falha de disco levam junto o trabalho de todos os clientes da instancia. E backup que existisse nao bastaria: **restauracao nunca testada e hipotese, nao backup** | Investigar por que nao sobe (`docker service ps pgbackweb_pgbackweb --no-trunc`) — pode ser trivial. Depois de ressuscitado, **testar uma restauracao de verdade**, em banco descartavel. Isso responde a pergunta 55, que o levantamento tinha promovido a mais importante da Parte B |
| R-67 | **Uma pessoa so conhece a infraestrutura, e ela e a mesma que responde pelo projeto.** Respondida a pergunta 57 em 05/09: somente o usuario tem acesso administrativo ao servidor e ao Portainer. Nao ha documentacao das 22 stacks, nao ha versionamento dos 194 fluxos (pergunta 46), e o backup do banco esta em laco de falha (R-66) | **Novo, moderado, e e o lado bom virado do avesso.** Uma pessoa so com a chave e o menor numero possivel — nao ha credencial de ex-colaborador esquecida nem conta compartilhada de administracao, o que **melhora** R-62 a R-64. O preco e que a infraestrutura inteira de um escritorio de advocacia passa a depender da disponibilidade de uma pessoa, sem ninguem que consiga reconstruir o que ela montou. Para o escritorio, o cenario nao e abstrato: e o dia em que houver prazo correndo e a plataforma parada | **A resposta nao e dar acesso a mais gente** — isso trocaria um risco por outro pior, e contraria a Regra 7. E reduzir o que so existe na cabeca de uma pessoa: documentar as stacks e como reconstrui-las, versionar os fluxos (ja e o caso da frente LEX, que nasce de codigo), consertar o backup (R-66) e guardar o acesso de emergencia em cofre, com procedimento escrito de quando se abre. Nada disso trava o projeto; entra na conversa de contrato, porque e o escritorio quem carrega a consequencia |
