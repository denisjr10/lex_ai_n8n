...
**Fechados:** Blocos 1, 2 e 3 inteiros. **O Bloco 3 fechou em 11/09.**
**Aguardando você:** o **Bloco 0** — e um dos três consertos do item 0.1 (a D-63) já saiu com o 3.1.
...
|---|---|---|---|
| 0.1 | **Não dar o aval em bloco (P-02) antes de três consertos:** riscar a parte revogada da D-63 (a D-144 já a derrubou), criar o bloco B19 no digesto com D-228/D-229/D-230 — que hoje não estão em grupo nenhum —, e reescrever o parágrafo de aval com os números do quadro §3.0 | 👤 🟠 **Um de três consertos feito em 11/09**: a D-63 teve o trecho revogado riscado e foi confirmada (item 3.1). Faltam o bloco B19 no digesto e o parágrafo de aval com os números do quadro §3.0 | É o único item da revisão que, se ignorado, produz **registro formal falso assinado por você**. ~20 min |
...
| # | Item | Estado |
|---|---|---|
| 3.1 | Tirar a **coluna de estado** das tabelas de decisão do PRD, da NT01 e do Modelo de Identidade; depois decidir uma a uma as nove divergentes → **D-234** | ✅ **Feito — as oito confirmadas, por decisão do usuário em 11/09.** D-18, D-21, D-61 e D-64 a D-67 entraram como ✅ Confirmadas na §13, citando o escritório em 27/08; a **D-63** também, com o trecho revogado pela D-144 riscado. A coluna de estado saiu do PRD §15.1 — a "Resposta do escritório", que é a prova, ficou —, e as duas células da NT01 voltaram a ser recomendação. O digesto ganhou aviso: o bloco B5 caiu de seis para uma (a D-62) |
...
| 3.14 | Dar **gancho operacional ao R-10 (OAB)**, com dono nomeado e data → **D-244** | ✅ **Feito — dona: Malu Souza; prazo: o da P-05**, por decisão do usuário em 11/09. A cláusula de uso de IA vai no mesmo aditivo que a de LGPD, antes do primeiro dado real. Registrado no R-10, na D-244, na P-05 do plano e na **§1.4 da pauta**, com linha na folha de respostas — sem isso, a dona nunca ouviria falar do item |
