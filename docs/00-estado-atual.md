...
| 🔴 **O que levar à Malu** | **Em ordem:** (1) **o aviso do Astrea** — ninguém vigia prazo e há assinatura sugerindo o contrário (R-75); (2) **horário útil** (20c), cinco definições com proposta, **falta a lista de feriados**; (3) **como vocês conferem prazo hoje** (pergunta 20) — agora é a **única** rede que existe; (4) a **fronteira** da D-212, antes de cancelar o Astrea |
...
## A revisão de 09/09 — o que foi aplicado e o que ficou

Uma revisão completa do repositório inteiro (22 documentos, 3.619 linhas de TypeScript, 13 migrações, os hooks, a demo e a suíte) produziu **60 achados**. O relatório completo está fora do repositório, com o usuário; o que importa aqui é o que já mudou e o que **ainda não**.

**A tese da revisão:** quase todo defeito encontrado é sintoma de **um só** problema de método — o projeto guarda o mesmo fato em vários lugares, escrito à mão, sem nada que confira se os lugares concordam. Foi assim que nasceram as cinco contagens de decisões, os quatro números errados no cabeçalho deste documento, as nove decisões com estado divergente entre documentos, o saldo em três arquivos e um hook, e o marco que é 7 num lugar e 8 noutro.

**O que já está aplicado (D-232 e D-233):** o contador, a sede única dos números, a conferência no `fechar-ciclo.mjs`, a CI, e o saldo lido do `06` em vez de copiado. Ver a linha do cabeçalho.

**O que a revisão achou e ainda NÃO foi corrigido** — em ordem de urgência:

| # | O quê | Onde |
|---|---|---|
| 1 | 🔴 **O aval em bloco do P-02 ratificaria a D-63 com a redação errada** — ela autoriza o agente do cliente a chamar a API paga, o inverso da **D-144**, e o bloco B5 do digesto pede o de acordo para "D-61 a D-64" sem avisar. Além disso **D-228, D-229 e D-230 não estão em nenhum dos três grupos** do digesto | §13 · `18-digesto-de-aval.md` |
| 2 | 🔴 **O R-02 (perda de prazo, único "Gravíssimo") tem como única mitigação uma rotina humana que o R-75 diz não existir**, e a pergunta 20 continua aberta. Enquanto ela não voltar, o cenário real é "E2 é a única camada, sem rede embaixo" | §15 |
| 3 | 🔴 **Nada no repositório verifica a assinatura do token de sessão.** A Spec §4.2 diz que a etapa 2 "valida assinatura"; `etapas.ts` confere datas e lista de revogação. Busca por `jwt`, `hmac`, `verify(`, `signature` em todo o código: só comentários. E o `13-chassi-marco-2.md` reescreveu a etapa sem essa metade e a deu por pronta | `packages/mcp-core/src/etapas.ts:62` |
| 4 | 🔴 **A faixa A4 não reconsulta o Policy Gate**, e o "advogado identificado nominalmente" é `!aprovacao.aprovador_id.trim()` — basta não ser vazio. O `policy-gate` é `export {}`. **Trava barata:** `A4_DISPONIVEL = false` na carga, como a A3a já faz | `etapas.ts:291` |
| 5 | ~~A Demo B afirma ao cliente uma revisão que não acontece~~ ❌ **ACHADO REFUTADO em 09/09, e vale registrar o erro.** A revisão apontou como descuido crítico o rodapé *"revisada por um advogado do escritório"* num fluxo sem pessoa nenhuma. **É a D-155**, decisão consciente e bem fundamentada: a demo mostra o texto de produção, o contexto é destinatária única que conduz a apresentação, e existe **trava mecânica** em `demo/testar-fluxo-b.mjs:461` que amarra a frase ao aviso `⚠️ DEMONSTRAÇÃO` — quem tirar o aviso para atender cliente real encontra o teste no caminho e tem de escolher entre a aprovação A3b e retirar a frase. **Por que o erro aconteceu:** a verificação adversarial da revisão (a lente "isto já está tratado?") morreu junto com o limite de sessão, e este achado chegou sem ela. Fica como aviso de método para quem ler o resto desta lista | — |
| 6 | 🟠 **A Regra 6 não tem trava em código:** não há etapa de custo no chassi, `definirFerramenta` não exige `custo` para a faixa A1 (a paga), e todo envelope devolve `custo_centavos: 0` | `mcp-core` |
| 7 | 🟠 **`identidade_externa` fica fora da política por linha.** A migração 010 justifica dizendo que a tabela está "pendurada num pai único, então não há o que divergir" — certo para integridade, errado para confidencialidade: a política não decide o que pode divergir, decide o que a consulta devolve | `dados/migracoes/010` |
| 8 | 🟠 **403 é traduzido como credencial inválida**, e o 403 real do Escavador é saldo bloqueado. O humano acionado vai rotacionar token em vez de recarregar. `resposta.mensagem` é recebido e nunca usado | `mcp-core/src/traduzir-erro.ts` |
| 9 | 🟠 **O guarda do Escavador protege quatro nomes de arquivo, não a capacidade** — script novo, interpretador fora da lista ou domínio em variável de shell passam. E provavelmente **falha aberto** se quebrar | `.claude/hooks/guarda-escavador.mjs` |
| 10 | 🟠 **O guarda de segredo tem falso positivo:** casa o padrão contra o texto cru do comando, então bloqueia quem **escreve sobre** um comando perigoso. Aconteceu ao gravar o relatório da própria revisão | `.claude/hooks/guarda-segredo.mjs` |

Fora isso, ~30 correções de documentação (estado divergente entre tabelas de decisão, o Marco 1 anunciando como garantia a regra que a migração 013 derrubou, o plano mandando levar à Malu uma pergunta já respondida, o `credentials/` que o `.gitignore` apaga em silêncio, nove RF sem critério de aceite, e os tetos de orçamento dimensionados para 5 advogados quando são 2).

**O que a revisão NÃO achou**, e num projeto com dado de cliente isso conta: nenhum segredo no histórico do Git em 119 commits, nenhum dado pessoal real versionado, nenhum SQL por concatenação, nenhum privilégio decidido por instrução em prompt. O projeto erra por duplicar informação, não por descuido de segurança.
