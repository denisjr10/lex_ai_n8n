...
| **2** | Antes do primeiro dado real de cliente — travas de carga, tetos no receptor, isolamento por inquilino | 🔶 **5 de 10 fechados** em `bc954d6` (D-238, D-240). Faltam as travas de faixa A4 e de sessão assinada, os tetos do receptor, a origem do callback e a escolha da D-239 |
| **3** | Documentação: parar de duplicar — 14 correções pontuais | ⬜ **Pendente**, ~7 h, divisível (D-234, D-235) |

**Uma ressalva de método que vale para tudo acima:** a revisão rodou em seis frentes paralelas e a **verificação adversarial não aconteceu** — a lente que pergunta *"isto já está tratado?"* morreu com o limite de sessão. Um achado já caiu por isso: o rodapé da Demo B que afirma *"revisada por um advogado do escritório"* era a **D-155**, decisão consciente com trava mecânica em `demo/testar-fluxo-b.mjs:461`. **Nada foi alterado na demo.** Todo item ainda não marcado ✅ merece a mesma pergunta antes de virar trabalho.
