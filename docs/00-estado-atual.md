...
| 🔴 **HÁ UM PRAZO, E ELE É O NOVO EIXO** | **10/09 é o ideal, 15/09 é o limite** para implementação e produção, em contratação de **escopo fechado** (perguntas 72 e 73, respondidas em 05/09). **As quatro frentes não cabem.** E1 e E2 são viáveis — fundação, chassi e auditoria já estão de pé e verificados, e a vigilância entrega publicação real por callback desde 27/08 a custo zero. **E4 (WhatsApp) não é**: não há conta, número nem template aprovado, e verificação de negócio na Meta leva semanas que não estão sob nosso controle (D-197). E3 depende da chave do Trello, que ainda não existe. **O saldo do Escavador só é contratado em 08/09** e hoje é zero. A proposta é **E1 + E2 em produção até 15/09**, com E3 e E4 em fase seguinte — e ela precisa do de acordo antes de começar, não no dia 14. Ver **D-195** |
