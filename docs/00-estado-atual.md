...
| ✅ **O BLOCO 3 DA REVISÃO FECHOU — 14 DE 14, E TRÊS PREMISSAS QUE NÃO ERAM O QUE PARECIAM** | **11/09**, em três lotes ([`20-revisao-geral.md`](20-revisao-geral.md)). Correções de documentação, cada uma conferida contra o disco **antes** de ser escrita. ✅ **O que entrou:** a D-145 marcada como revisada pela D-194 na §13, no PRD e na Spec; o receptor como **marco 8** em quatro lugares que diziam 7; o Marco 1 com a regra da migração 013 riscada; a árvore do repositório com um só dono (Spec §3); o `hash` como índice e o JSON de preços copiado virando ponteiro; cinco riscos com número derrubado; os sete estados escritos no cabeçalho da §13; a seção de riscos daqui virando ponteiro; PRD 2.1, plano 1.1, README sem número nenhum; e critério de aceite para nove requisitos. ⚠️ **Três premissas ajustadas:** o conflito da D-145 é com a **RF-13**, não com a D-194 no assunto; eram **quatro** "marco 7", não cinco; e o calendário provisório **não foi escrito** — ele já existe na pauta 21. 🔴 **Achado de passagem:** **nenhum rito de alerta está no banco** — a tabela `alerta` tem `lido_por` único, sem confirmação por papel nem janela de reversão. ✅ **E as duas que dependiam de você:** as **oito decisões** que o escritório confirmou em 27/08 entraram como Confirmadas na §13 — a D-63 com o trecho revogado pela D-144 riscado —, e o **R-10** ganhou dona e prazo: a **Malu**, no mesmo aditivo da cláusula de LGPD (P-05) |
| ✅ **O BLOCO 2 DA REVISÃO FECHOU, E O RECEPTOR PASSOU A CONFERIR A ORIGEM** | **10/09** (**D-245**, **R-90**, migração **016**). A Spec §8.1 sempre listou *"Validar a origem"* como a **etapa 1** do receptor, com *"falha fecha"* ao lado — e o receptor **não validava**: recebia um booleano decidido por um nó do n8n e acreditava. O problema não era o n8n, era o **formato**: quem chama decidindo se a origem é válida é o mesmo defeito que a **D-237** tratou no chassi. E o veredito trafegava pelo histórico de execução do n8n, do qual **98,5% dos registros já sumiram** — a prova da conferência era mais efêmera que o fato conferido. ✅ **Os cabeçalhos sempre estiveram lá:** o nó os carimba desde 26/08, e era o *recolhimento* que os jogava fora. Agora o serviço lê o segredo do ambiente, compara em tempo constante, e **o veredito de fora nunca promove entrega que nós recusamos** — antes bastava dizer *"é válida"* para a entrega virar publicação, e publicação alimenta alerta de prazo. Sem segredo configurado, `origem_valida` é falso (Regra 5), e `origem_conferida_por` registra quem decidiu cada linha. 🔴 **Achado novo, o R-90:** o segredo do callback está **escrito dentro do nó do n8n**, numa instância com 194 workflows de terceiros — e a **R-38** diz que a chave daquele n8n não tem escopo. Mitigado pela metade: o nó deixou de ser a única barreira. Falta **girar o segredo** e **tirá-lo do nó** no marco 8. **141 testes**, doze novos |
| ✅ **O CONTEÚDO EXTERNO PAROU DE ENTRAR SEM TETO** | **10/09** (**D-241**, migração **015**). A **Regra 4** diz que conteúdo externo é hostil, e até hoje ele era tratado como bem-comportado: `teor` sem limite, nome de envolvido sem limite, lista de envolvidos percorrida inteira, e a função que estabiliza o JSON para a chave de idempotência **recursiva sem limite de profundidade**. ✅ Tetos: teor **200 KB**, nome **300 caracteres**, **200 envolvidos**, aninhamento **32 níveis**. 🔴 **A regra que governa tudo isso: estourar NÃO descarta.** O que passa é cortado e registrado, e a publicação segue para a base — descartar por tamanho repetiria, pela outra ponta, o defeito da migração 013, em que **seis intimações de seis processos diferentes sumiram sem erro nenhum**. ⚠️ **A recomendação da revisão era gravar com `estado = 'truncado'`, e ela não funciona:** `estado` é o ciclo de vida, e o índice de trabalho pendente é `WHERE estado = 'recebido'` — o evento marcado `'truncado'` sairia do índice e **nunca seria processado**, virando uma forma nova de descarte em silêncio. A truncagem virou **coluna própria** (`jsonb`, dizendo o que estourou e por quanto, para calibrar os tetos depois contra a realidade em vez de defendê-los). 🔴 **O teto de profundidade é o mais sério dos quatro:** ele não contém volume, contém **travamento** — a estabilização roda em toda entrega, antes de qualquer gravação, e sem teto um JSON aninhado de propósito derruba a pilha no ponto em que o receptor ainda não registrou nada. Receptor que morre antes de registrar não deixa nem o sinal de que foi atacado. **129 testes**, nove novos |
| ✅ **AS DUAS ÚLTIMAS TABELAS DE DADO DE CLIENTE ENTRARAM NA POLÍTICA POR LINHA** | **10/09** (**D-239**, migração **014**). `identidade_externa` e `reserva_orcamento` ficavam de fora desde a migração 010, com um argumento que parecia bom: *"penduradas num pai único, então o escritório delas é o do pai, e não há o que divergir"*. A primeira metade é verdadeira e **a conclusão não segue dela** — *"não há o que divergir"* fala de **integridade**, e política por linha existe para **confidencialidade**. Um `SELECT * FROM identidade_externa WHERE identificador_externo = $1` **sem `JOIN`** devolvia a linha de qualquer escritório, sem erro nenhum. ✅ As duas ganharam `inquilino_id`, referência composta ao pai e política: **20 de 23 tabelas** sob política agora, eram 18. ✅ **A porta única do login:** isolar `identidade_externa` cria um problema real — o login acontece **antes** de o escritório ser conhecido —, e a saída não foi deixar a tabela aberta, e sim `identidade_para_login()`, função `SECURITY DEFINER` com `search_path` fixo, revogada de `PUBLIC`, que devolve **só** `inquilino_id` e `usuario_id`. Identidade revogada e usuário desligado devolvem vazio, indistinguível de desconhecido. 🔴 **O que deliberadamente NÃO mudou:** o índice de unicidade da identidade continua **global**. Torná-lo composto era o movimento automático e destruiria a **Regra 7** — o mesmo WhatsApp viraria pessoa diferente em dois escritórios, em silêncio (R-11). O isolamento é da leitura; a unicidade da identidade é do sistema inteiro. **52 provas de regra**, seis novas, e uma delas estava passando **pelo motivo errado** depois da migração: recusava por `NOT NULL` em vez de pelo índice que ela existe para provar |
| ✅ **AS DUAS TRAVAS DE SEGURANÇA DO BLOCO 2 ENTRARAM — E AS DUAS MUDARAM DE FORMA NO CAMINHO** | **09/09** (**D-236**, **D-237**, **R-89**). Os dois furos que a revisão chamou de críticos foram fechados até onde o marco 9 permite. **A faixa A4 não executa** (D-236): `etapaAprovacao` recusa depois de conferir tudo o mais, porque as duas garantias que a A4 promete — reconsulta ao Policy Gate no ato e advogado identificado nominalmente — não existem em código. **A sessão passou a ter procedência declarada** (D-237): `origem_da_sessao` é campo obrigatório e sem padrão, e declarar `'verificada'` é recusado enquanto o chassi não souber conferir assinatura. Nenhuma das duas conserta o buraco de fundo — o chassi ainda recebe `Sessao` pronta de quem chama, que é a Regra 1 ao contrário —, mas as duas impedem que a lacuna siga **invisível** e que alguém afirme garantia que o chassi não cumpre. ⚠️ **As duas foram implementadas primeiro como a revisão pedia, e as duas tiveram de mudar:** a trava de carga da A4 derrubou **47 dos 118 testes**, e o `13-chassi-marco-2.md` que a revisão mandava corrigir **já estava certo** — quem superdeclarava era a Spec §4.2 e §5.3. 🔴 **Com isso são três os achados da revisão que não se sustentaram**, todos pela mesma causa: a verificação adversarial não aconteceu. A tabela dos três está na ressalva de método do [`20-revisao-geral.md`](20-revisao-geral.md), e a regra prática é ler o arquivo antes de aceitar o achado. 🆕 **De quebra, a barreira dos números estava parcialmente desligada** (**R-89**): o `contar.mjs --conferir` isentava da comparação qualquer linha que **contivesse** travessão, e travessão é pontuação comum — pacotes, documentos e riscos nunca foram conferidos, e a contagem de documentos ficou dias em 23 com 24 no disco. Corrigido: só isenta a linha cujo **valor** é o travessão. É a mesma correção do guarda de segredo (R-88), pela mesma causa |
| ✅ **OS NÚMEROS DO PROJETO PARARAM DE SER ESCRITOS À MÃO** | **09/09** (**D-232**, **D-233**). Uma revisão completa do repositório encontrou **o mesmo defeito em dezessete lugares**, e ele é sempre o mesmo: um fato contável guardado à mão em quatro ou cinco documentos, divergindo. A pilha de decisões tinha **cinco contagens diferentes, todas datadas de 08/09** — 215, 225, 222, 182 e 187 — contra **231 no disco**, e três delas não fechavam nem consigo mesmas (179 + 31 = 210, não 215). O cabeçalho deste documento dizia *"10 migrações, 45 provas de regra, 92 testes"* contra **13, 46 e 109**, e a decomposição não somava. O README dizia 7 e 44. Até o passo criado para corrigir isso — o **P-19** — carregava números errados. ✅ **A D-232 muda a causa, não os sintomas:** `ferramentas/contar.mjs` conta do disco e gera [`numeros.md`](numeros.md), que passa a ser a sede única; o hook `fechar-ciclo.mjs` **recusa encerrar a sessão** com o arquivo velho, e a **CI** (`.github/workflows/verificar.yml`, nova) confere a cada envio, junto com `npm run verificar`. Campo não medido sai como `—`, **nunca como zero**. ✅ **A D-233 faz o mesmo com o saldo do Escavador:** o cabeçalho do [`06`](06-orcamento-de-chamadas-escavador.md) vira a sede, o `CLAUDE.md` deixa de copiar o número, e o hook `estado-do-repo.mjs` passa a **ler** em vez de repetir — ele trazia *"R$ 47,00, validade até 01/09/2026"* cravado no código, com a cota **expirada desde 01/09** e o saldo em **R$ 0,00**. Junto subiram para o início de toda sessão duas coisas que estavam enterradas: que **403 do Escavador é saldo bloqueado** (o `CLAUDE.md` mandava lê-lo como problema de rede) e que a **assinatura mensal `2813617` continua ativa e precisa sair até 22/09**, antes de qualquer recarga. 🔶 **O resto da revisão tem roteiro próprio e vivo:** [`20-revisao-geral.md`](20-revisao-geral.md), com o estado de cada achado e a prova de quem já foi feito. Metade do Bloco 2 fechou em 09/09 (`bc954d6`, **D-238** e **D-240**); continuam abertas as travas de **faixa A4** e de **sessão assinada** — os dois furos críticos de segurança —, os tetos do receptor e a escolha da **D-239**. A frase da Demo B **não era achado**: é a D-155, e o registro do erro ficou |
| 🔴 **O ALERTA DE PRAZO NÃO TINHA ONDE SER GERENCIADO, E AGORA TEM** | **08/09** (**D-226** a **D-231**, **RF-56 a RF-64**, **R-80 a R-83**, PRD §5.2.2). Pergunta do usuário: *"quando chega um alerta de movimentação ou de prazo, já está previsto que esse alerta seja cadastrado no Trello como tarefa?"* **Não estava — nem um pouco.** O único requisito que criava card era a **RF-20**, e o gatilho dela é e-mail de demanda (E3); a tabela `alerta` não tinha campo nenhum ligando alerta a tarefa. E o buraco estava escrito no nosso próprio PRD, no §5.2.1: *"a plataforma não acompanha isso"* — o rito entregava **prova de ciência** e parava aí, e depois do "Ciente" o prazo voltava para a memória das pessoas, que é exatamente a rede que a **R-75** diz não existir. ✅ **A D-226 corrige:** todo alerta com indício de prazo gera uma `tarefa` na base, espelhada como **card no Trello**, e entra a **fatia mínima de E3** — 3 das 12 ferramentas, os campos personalizados e um webhook. 🔴 **A D-227 é a condição de isso não violar a RF-11:** a **data final do card é a data-limite de TRIAGEM**, jamais o prazo processual — card com data de vencimento escrita por automação **é** a plataforma afirmando prazo, e afirma pior que num texto, porque fica vermelho e entra no calendário. Etiqueta `prazo não calculado` no nascimento; quem escreve o prazo real é advogada, à mão. **Alcance escolhido: prazo agora, movimentação depois de medir** (D-228). ⚠️ **O preço está escrito:** **R-83** — isto é **mudança de escopo em contrato fechado**, E3 saiu da janela pela D-195 com o de acordo da Malu, então vai a ela **junto com a D-219** (passo **P-28**); **R-80**, o quadro verde parece prazo controlado sem ninguém ter contado; **R-81**, o Butler é desconhecido e reage à primeira escrita, agora com bloqueio **em código** (RF-62); **R-82**, duas filas disputando a mesma gestão. 🚧 **Quatro perguntas do escritório subiram para bloqueadoras de E2:** **24, 25** (quadros e fluxo do card — nunca foram enviadas) e **26, 27** (campos personalizados e Butler). **A entrega tem duas metades** (P-29): a tarefa na base **não depende de nada disso** e cabe em 15/09; o card liga quando a chave chegar |
| ✅ **A colisão de numeração dos RF acabou** | **08/09** (**D-231**, fecha o **P-18**). RF-42 a RF-45 estavam cada um definido **duas vezes** — catálogo de gabaritos (§6.3) e vigilância (§9.3). O bloco de gabaritos ficou, por ser o mais antigo e o mais citado fora do PRD; **o de vigilância andou para RF-49 a RF-55**, e a numeração voltou a subir junto com o documento. **15 referências cruzadas em seis documentos** foram atualizadas. A tabela de correspondência está em [`17-plano-de-execucao.md`](17-plano-de-execucao.md) §12.1 — quem reler documento anterior a 08/09 vai encontrar os números antigos. 🔴 **Falta a prevenção**, e é a mesma de sempre: um hook que recuse commit com identificador repetido. Foi a **terceira** ocorrência do defeito |
| 🟠 **O PAINEL WEB NÃO EXISTE, E TRÊS DECISÕES ACORDADAS DEPENDIAM DELE** | **08/09** ([Nota Técnica 04](19-painel-web-e-administracao.md), **D-223** a **D-225**, **R-78**, **R-79**). Pergunta do usuário: *existe previsão de painel ou interface web, e por onde o super admin gerencia?* A **D-16** prevê um painel desde 17/08 — e ele **não é nenhum dos dez marcos da Spec §15**, não tem especificação, telas nem estimativa. Enquanto isso, **três decisões foram pendurando peso nele por referência**: a **D-147** (*"a identidade sai do Google e passa a viver no painel web"*), a **D-17** (conteúdo confidencial não vai na mensagem, só o **link** — e o link aponta para o painel) e o texto da **RF-01** no documento 04 (*"o login por senha do painel é obrigatório desde a primeira versão"*). **A causa foi forma, não descuido:** cada decisão herdou o vocabulário e supôs que ele nomeava algo planejado. ✅ **A D-223 separa o painel em três camadas e as data:** camada 0 (o vínculo Telegram ↔ `usuario`, que existe e **cumpre o critério de aceite da RF-01**), camada 1 (login + 2FA, 3 a 5 dias), camada 2 (caixa de aprovações, 5 a 8 dias) e camada 3 (console de admin). **Nenhuma entra em E2 nem em E1** — as camadas 1 e 2 vão juntas para depois de 30/09 (P-26), porque login que não abre nada não vale o esforço. ✅ **A D-224 responde o super admin: é o prestador, por acesso técnico direto** — n8n, Portainer, pgAdmin, painel do Escavador —, e **não existe console**; a camada 3 entra quando o escritório assumir a administração. ✅ **A D-225** cumpre **RF-08** e **RF-37** por **relatório gerado**, não por tela — e é o único trabalho novo antes de 30/09, já dentro do escopo da D-219. 🔴 **O que fica descoberto está escrito:** **R-78** — a D-17 manda o conteúdo para um link que não existe, e a pressão para pôr teor no Telegram cresce a cada uso (tratamento imediato e barato: **P-27**, escrever a regra do que pode aparecer no corpo da mensagem); e **R-79** — o escritório **não consegue desligar ninguém sem o prestador**, que é a necessidade N9 da Nota Técnica 01 sem dono, somada ao R-47 |
...
| 🟢 **O rito do alerta de prazo fechou, e o escritório escolheu o lado seguro** | **07/09** (**D-208**, **D-209**). N1 de **2 h úteis**, N2 de **4 h úteis até a Malu**, expiração de aprovação em **4 h/2 h**. E a 20e escolheu **REABRIR** em vez de fechar: **o R-68 cai** — o silêncio da advogada volta a não significar nada, e a RF-15 e a linha **Nunca** do PRD §5.2.1 valem de novo sem exceção. ⚠️ Em troca nasceu o **R-76** (era R-70): alerta que reabre de 4 em 4 horas vira ruído. A D-209 resolve separando **estado** de **aviso** — reabrir muda o estado e **não reenvia**; o alerta vira *pendente de advogada* e aparece **uma vez por dia** no resumo. 🚧 **Tudo isso está inerte até a 20c** (horário útil) fechar: "2 horas úteis" precisa de um relógio que diga o que é útil |
| 🟢 **A pilha de aval foi triada — 179, não 173, e o digesto existe** | **08/09** (**P-17**, [`18-digesto-de-aval.md`](18-digesto-de-aval.md)). A §13 tem **184 em 🟡 Proposta** e **36 confirmadas ou resolvidas** (uma delas marcada 🔄 Revisada) (recontado linha a linha em 08/09: **225 linhas, 225 identificadores distintos**) — eram 179 antes de a própria conversa gerar D-216 a D-218, e 182 antes de a pergunta sobre o painel gerar D-223 a D-225 — a conta antiga de "173 contra 2" estava errada dos dois lados. Elas saíram em três grupos: **17 que ainda pedem leitura individual** — eram 15, cinco foram decididas e sete nasceram desde então —, **159 de acordo em bloco** e **8 que não deveriam estar em Proposta** (17 + 159 + 8 = 184). **v2.0, de 08/09:** cada ficha do grupo A ganhou contexto, exemplo concreto, os números em reais e uma linha de decisão para marcar; os 18 blocos passaram a dizer o que decidem em vez de nomear o assunto; e entrou um **glossário** do vocabulário do projeto. Nenhuma recomendação mudou — só a forma de apresentar. ✅ **Duas coisas já foram resolvidas no mesmo dia:** a colisão de numeração (linha acima) e a **D-171**, que não era exceção esquecida e sim decisão de negócio — virou **D-216** (linha acima). **Nenhum outro status foi alterado** — o aval é do usuário |
| ✅ **Os cinco primeiros avais chegaram, e a data se move em vez de o escopo encolher** | **08/09.** **D-216, D-182, D-206 e D-209 confirmadas**; **D-207 recusada**. A decisão que reorganiza tudo é a recusa do recorte: *"não recortar, e negociar mover a data de 15/09, para que possamos fazer tudo e entregar completo e verificado"*. ❌ **O calendário da §10 do plano caiu junto** — ele assumia a D-207 aceita. O que vale é a **D-219**: **faseamento**, com **E2 inteira em 15/09** e **E1 completa em 30/09** (17 dias úteis), e **cache, painel de custo e relatório de acesso amplo de volta ao escopo**. 🚧 **Depende do de acordo da Malu** — ela acordou 15/09 em 05/09, e mudar isso é conversa, não aviso. E3 e E4 continuam fora (D-195). 🆕 **Três perguntas feitas ao dar o aval acharam buracos reais no desenho:** como um processo novo entra na triagem (**D-220** — três portas, e a principal é a própria fila), como se cadastra um advogado novo (**D-221** — duas metades, R$ 3,00/mês recorrente, e a franquia que não se edita depois), e como a advogada confirma um alerta na prática (**D-222** — botão, e no segundo ciclo são dois, com significados diferentes) |
| 🟠 **As atendentes aprovam mensagem ao cliente — e isso é decisão da Malu, não sobra da demo** | **08/09** (**D-216**, **D-217**, **D-218**, **R-77**). O digesto tinha lido a **D-171** como exceção de demonstração esquecida no disco e recomendado reverter. **Estava errado:** as duas colaboradoras foram **contratadas como atendentes**, e a advogada proprietária adota a autonomia delas deliberadamente — sem isso, ou a Malu responde todos os clientes o tempo todo, ou ninguém responde. A **D-216** registra isso no lugar certo, com a fronteira escrita: mensagem que **informa** qualquer perfil aprova; mensagem que **compromete** (assume prazo, promete resultado, reconhece fato, orienta juridicamente) continua exigindo advogada. ⚠️ Custa afrouxar a conferência de papel em **A3b** no chassi (D-156 item 2) — **decisão registrada, nunca por omissão**. **A pergunta que veio junto vale mais que a resposta:** avisar a advogada a cada aprovação não vira entulho a ponto de ela ignorar tudo, inclusive prazo? **Vira** — daí a **D-217**: tempo real no Telegram só para o que tem relógio (alerta de prazo e encerramento A4a, ~5/dia útil), o resto vai para o **resumo de fim de dia útil**, e fura o resumo só a aprovação que toca uma regra. *Esclarecimento de fato: o "avisar a cada geração" da D-171 é `console.log` do gerador — nunca chegou ao Telegram de ninguém.* A **D-218** é o que resolve o volume de verdade: com o catálogo de gabaritos de pé (A3a), a rotina sai sob texto aprovado uma vez e nem entra na fila |
...
| ✅ **A porta do banco: exposta, e sem sinal de exploração** | **05–06/09.** `pgbouncer *:5432` e `postgresbackupmcp *:5433` **confirmados abertos à internet** por teste externo (**R-63**) — em Swarm a porta publicada atravessa o `ufw` mesmo. A busca por invasão foi refeita **no lugar certo** (pgbouncer, não PostgreSQL) e deu **zero tentativa de autenticação falha**; só apareceu `closing because`, 88 vezes, que é linha normal. **Duas ressalvas fazem parte do achado:** a janela é de **~33 h**, e **ninguém ter tentado não é estar protegido** — mediu-se que a porta ainda não foi encontrada. Muda a urgência, não a necessidade: firewall como remendo, **túnel SSH no pgAdmin** como solução. 🔴 **O Portainer público (R-62) passou a ser o item mais grave em aberto** |
| 📘 **A descoberta está em 83 respostas** | **08/09.** Fecharam **A1**, **A5**, **A6**, **B1**, **B2** e **C** inteiras; **A3** voltou parcial. Continuam abertas: **A2 inteira** (7–14), **A4** (24, 25, 28, 29) e, da A3, as **15, 17, 18, 19, 20 principal, 21 e 22**. ⚠️ **O formato do documento mudou em 06/09** — a pergunta volta em negrito e a resposta fica recuada abaixo, e **a pergunta nunca é apagada quando a resposta chega**: sem ela a resposta não se entende, e o documento vai ao escritório |
| 🔴 **O que levar à Malu** | ✅ **Consolidado em 09/09 na [`21-pauta-reuniao-malu.md`](21-pauta-reuniao-malu.md)** — a pauta única da reunião, com **toda** pendência que depende dela, na ordem em que a conversa deve acontecer, e uma folha de respostas na §12. **A ordem de valor:** (1) **o aviso do Astrea** — ninguém vigia prazo e há assinatura sugerindo o contrário (R-75); (2) a **fronteira** da D-212, antes de cancelar o Astrea; (3) **as datas** da D-219 e a **mudança de escopo** da D-226; (4) as providências que travam a construção — chave do Trello, sete Telegram, lista da Ana Beatriz, saldo do Escavador; (5) **franquia de aparições** (irreversível) e **horário útil** (20c), que **falta a lista de feriados**; (6) **como vocês conferem prazo hoje** (pergunta 20) — agora é a **única** rede que existe |
...
| 🟢 **A vigilância ganhou nome, e o advogado compartilhado ganhou desenho** | **05/09** — D-205 revisada pela **D-206**, PRD §9.3.2 e §9.3.3. **As duas advogadas vão de V1** (nome no diário oficial): Malu e Ana Beatriz, **R$ 3,00/mês cada**. O V2 por processo caiu para a Ana Beatriz — custava 20× mais (R$ 55,20–72,00) e cobria menos, porque só vê processo cadastrado. O problema real é que ela **atua em outros escritórios** e a Malu acompanha só ~24 dos processos dela. **Quatro medições reduziram o problema:** a franquia é **por monitoramento** (o ruído de uma não cega a outra); o volume real é de **~5 aparições por dia útil para ~289 processos**, o que projeta ~27–45/mês no total da Ana Beatriz; o `origens_ids` **já restringe aos 5 diários do Amapá**; e 🔴 **o `envolvidos` não serve de filtro — a Malu aparece nele em só 28 de 34 aparições do próprio monitoramento dela.** O **CNJ veio em 34 de 34**. Desenho: **triagem de pertinência pelo CNJ, no receptor**, com três saídas — acompanhado, não acompanhado (registro mínimo, sem conteúdo) e **desconhecido → fila humana, nunca descarte**. Gera **RF-52 a RF-55** e **R-70**. Por Regra 3, não entra no MCP |
| 🟢 **A descoberta fechou quase inteira** | **05/09** *(superado — ver a linha "83 respostas", de 08/09)*. Na época, **63 respondidas** — a seção A1 pelo escritório, as Partes B1 e C por você, e as 41 a 50 **por medição**, com a chave de API do n8n. Nasceram **D-192 a D-200** e **R-59 a R-61**. A responsável interna é a advogada **Malu Souza** (D-200). O que sobra: as sete de infraestrutura (51–57), a chave do Trello, e as Partes A2 a A6, que ainda não foram ao escritório |
| 🔴 **A INFRA FOI MEDIDA, E TEM O QUE CONSERTAR ANTES** | **05/09**, no terminal do servidor. Hostinger, Ubuntu 22.04, **2 vCPU / 7,8 GB / 96 GB**, **Docker Swarm com 22 stacks** — não `docker compose`, como se supunha. **Treze serviços estão publicados na internet, e três não deveriam** (R-62): **Portainer** (o plano de controle do Docker inteiro), **Prometheus** (que não tem autenticação por padrão) e **pgbackweb** (backup e restauração de banco). Mais: **o backup do PostgreSQL está instalado e PARADO** — 0 de 1 réplica (R-66), e é nesse banco que vivem os 194 fluxos sem versionamento. **Os segredos ficam em variável de ambiente**, sem cofre, e o Swarm tem um (R-64). E 🔴 **`pgbouncer` publica a porta 5432 no host — CONFIRMADO ABERTO À INTERNET** em 05/09, por teste de fora da rede (R-63): a única coisa entre a internet e o banco é a senha. Nada indica que alguém tenha entrado — nem o contrário; conferir os registros de autenticação é o próximo passo. ✅ **06/09: a 5433 também está aberta**, e o uso legítimo da 5432 ficou conhecido — o usuário acessa o banco por **pgAdmin**, o que faz do túnel SSH a saída certa em vez de derrubar a porta. ⚠️ **A busca por tentativa de invasão olhou o lugar errado:** quem atende a 5432 é o **pgbouncer**, e é no registro dele que quem bate na porta aparece — a leitura vazia do `postgres_postgres` **não** significava "ninguém tentou". ✅ **Refeito no lugar certo em 06/09: ZERO tentativa de autenticação falha** — só `closing because` apareceu, 88 vezes, e ele é linha normal de conexão fechando. Com duas ressalvas que são parte do achado: a janela é de **~33 horas**, e **ninguém ter tentado não é estar protegido** — mediu-se que a porta ainda não foi encontrada. Muda a urgência, não a necessidade: firewall esta semana, **túnel SSH no pgAdmin** como solução. ⚠️ **O enxame tinha DOIS nós**, e `srv1093898` **vai ser removido** (sessão própria) — antes disso, conferir em que nó as tarefas do n8n rodam: ✅ **Verificado em 06/09: o receptor NÃO é afetado** — as duas réplicas do webhook, o worker e o editor já estão em `srv957606`. Só o `n8n_mcp_api` está no nó que sai, e ele não é deste projeto. **Sem janela crítica nossa**; o procedimento é drenar, confirmar, remover. Nada disso trava o projeto; tudo isso precede dado real do escritório, porque sob a D-148 a infra é do prestador, o que o torna **operador de dados** perante a LGPD |
| ✅ **Três números que faltavam** | **05/09.** (1) **A retenção de execução é de 14 dias OU 10.000 execuções**, o que vier primeiro — o **R-57 deixou de ser estimativa**, e num servidor com fluxos comerciais ativos o teto de 10.000 chega antes. A janela é curta **e variável com o movimento de outros clientes**, o que é pior que curta: o marco 7 sobe de prioridade (D-203). (2) **Teto de 16 MB por requisição** — autos em PDF passam disso, e o desvio é o MinIO, que já está no ar (D-204). (3) **O n8n conecta ao banco como superusuário `postgres`** — então a plataforma **não** compartilha instância nem credencial com ele, ou a auditoria imutável do marco 3 seria contornável pela porta dos fundos (D-201) |
| ⚠️ **O WhatsApp não oficial já está no ar** | **05/09.** `evolution-api v2.3.1`, `wuzapi` e `chatwoot v4.1.0` rodam no servidor, com endereço público. Até aqui D-10 e P-05 recusavam biblioteca não oficial por risco de **banimento do número**, e recusar custava pouco porque a alternativa era abstrata. **Agora não é**: a Evolution sobe um número hoje, e é a **única opção de WhatsApp que cabe no prazo da D-195**. Minha posição continua sendo **tirar E4 desta fase** em vez de escolher entre as duas — mas a escolha é do usuário e da Malu, e escondê-la seria pior que fazê-la (**D-202**) |
| 🔎 **A instância n8n foi medida** | **05/09** — `16-levantamento-instancia-n8n.md`. **`v1.123.18`**, self-hosted, **queue mode confirmado por métrica**, PostgreSQL, todos os nós de IA e MCP disponíveis. **194 fluxos, 9 ativos — e cinco dos nove são de outros clientes do prestador.** Dois achados que ninguém pediu: **R-60**, `/metrics` responde sem autenticação e entrega a versão exata; e **R-61**, existe uma credencial da API do próprio n8n ("Conexão Geral") em dois fluxos de produção de terceiros, e chave de n8n **não tem escopo** — alcança os 194 fluxos e todo o histórico de execução, incluindo os nossos. **Sem versionamento nenhum** na instância; a frente LEX escapa porque seus fluxos nascem de código versionado |
...
| Código | ✅ **Fundação, chassi e auditoria de pé, os três verificados.** 📊 **Os números estão em [`numeros.md`](numeros.md)** — gerado por `node ferramentas/contar.mjs`, e a partir de 09/09 é a **sede única** deles (D-232). *Não copie número de lá para cá: esta linha trazia "10 migrações, 45 provas e 92 testes" contra 13, 46 e 109 no disco, e a própria decomposição não somava.* Monorepo com 9 pacotes — **4 implementados, e 5 ainda são casca (`export {}`)**: os dois servidores MCP, os dois SDKs e o Policy Gate, que são os marcos 6, 7 e 9. `npm run verificar` roda migrações, regras, auditoria e testes; desde 09/09 a **CI roda isso sozinha a cada envio**. O banco recusa conta compartilhada, alteração de auditoria, estouro de orçamento e evento duplicado. Mais: captura, importador de autos em PDF, anonimizador, cliente do n8n e **as duas demos rodando** — A no Telegram (`ZPh3DxptHFIyWETO`, **31 nós, 162 verificações**) e B no WhatsApp (`Hxc7uAmAUhyPE7E1`, **15 nós, 106 verificações** — a recusa agora chama um advogado no Telegram, D-163), **ligadas uma na outra**: aprovar no Telegram envia ao cliente |
...
## A revisão de 09/09 — o roteiro vive em `20-revisao-geral.md`

Uma revisão completa do repositório inteiro produziu **60 achados**, e o **roteiro de execução deles é [`20-revisao-geral.md`](20-revisao-geral.md)** — documento vivo, com o estado de cada item, a prova de quem já foi feito e o próximo passo. **Esta seção não repete aquela tabela**: número e estado guardados em dois lugares divergem, que é o defeito que a própria revisão encontrou em dezessete lugares (D-232).

**A tese da revisão:** quase todo defeito encontrado é sintoma de **um só** problema de método — o projeto guarda o mesmo fato em vários lugares, escrito à mão, sem nada que confira se os lugares concordam.

**Onde a execução está** (detalhe e prova em [`20-revisao-geral.md`](20-revisao-geral.md)):

| Bloco | O quê | Estado |
|---|---|---|
| **0** | Antes da conversa com a Malu — aval do P-02, lista de assuntos, R-02 travado, assinatura `2813617` | 👤 **Pendente, e é decisão sua.** O item 0.1 é o único da revisão que, se ignorado, produz registro formal falso assinado por você |
| **1** | A hora que eliminou uma classe de defeito — contador, sede única, conferência no `fechar-ciclo`, `CLAUDE.md`, CI | ✅ **Fechado em 09/09** (D-232, D-233, D-242) |
| **2** | Antes do primeiro dado real de cliente — travas de execução, tetos no receptor, isolamento por inquilino | ✅ **FECHADO em 10/09**, 10 de 10 — `bc954d6` (D-238, D-240), `b71f494` (D-236), `917baaf` (D-237), migração **014** (D-239), **015** (D-241) e **016** (D-245, R-90) |
| **3** | Documentação: parar de duplicar — 14 correções pontuais | ✅ **FECHADO em 11/09**, 14 de 14 — o 3.1 e o 3.14 por decisão sua |

**Uma ressalva de método que vale para tudo acima:** a revisão rodou em seis frentes paralelas e a **verificação adversarial não aconteceu** — a lente que pergunta *"isto já está tratado?"* morreu com o limite de sessão. **Três achados já caíram por isso** — a frase da Demo B (era a **D-155**, com trava mecânica em `demo/testar-fluxo-b.mjs:461`; nada foi alterado na demo), a premissa da faixa A4 (*"não existe ferramenta A4 nenhuma"* — existe, e é o alicerce da suíte) e o `13-chassi-marco-2.md` (que já escopava a etapa 2 corretamente). Todo item ainda não marcado ✅ merece a mesma pergunta antes de virar trabalho, **e ela se responde lendo o arquivo, não relendo o achado**.

**O que a revisão NÃO achou**, e num projeto com dado de cliente isso conta: nenhum segredo no histórico do Git, nenhum dado pessoal real versionado, nenhum SQL por concatenação, nenhum privilégio decidido por instrução em prompt, nenhum teste falhando. O projeto erra por **duplicar informação**, não por descuido de segurança.
...
| **Digesto de aval — as 182 propostas triadas em 15 para ler, 159 para bloco e 8 para fechar, com glossário** | `18-digesto-de-aval.md` |
| **Nota Técnica 04 — o painel web em três camadas, e por onde o super admin gerencia** · D-223 a D-225, R-78 e R-79 | `19-painel-web-e-administracao.md` |
...
- 12 ferramentas curadas sobre 261 operações — a superfície destrutiva do Trello (excluir quadro, apagar histórico, arquivar em massa) fica fora de todo perfil. ⚠️ **Mudou em 08/09 (D-226):** três delas — `criar_card`, `atualizar_card`, `mover_card` — **subiram para E2**, para o alerta de prazo virar tarefa no quadro. `arquivar_card` **não** subiu, e a ausência é deliberada: nesta fase nada some do quadro por ação da plataforma.
...
**D-01 a D-222** estão em `01-diretrizes-gerais.md` §13 — registro único e centralizado.

> ✅ **08/09 — a pilha foi triada, e o digesto existe.** O passo **P-17** fechou: [`18-digesto-de-aval.md`](18-digesto-de-aval.md) separa as propostas em **três grupos** — as que pedem leitura individual, as de acordo em bloco e as que não deveriam estar em Proposta. ⚠️ **Os números desta linha envelheceram no mesmo dia** e a contagem viva é a da §1 do próprio digesto: **184 em Proposta**, com **17 fichas individuais** depois das D-219 a D-225. **Nenhum status foi alterado** — o aval é do usuário, e é ele que destrava o **P-02**, o PRD e a Spec.
...
|---|---|
| 🟡 Proposta | **181** |
| ✅ Confirmada / Resolvida | **35** — quatro entraram com os avais de 08/09 |
| 🔴 Em aberto | 3 — **D-197, D-202, D-212** |
| 🔄 Revisada | 1 — D-205 (estado fora da convenção de três; ver digesto §6) |
| ❌ Derrubada | 1 — D-153 |
| **Total** | **222 linhas, 222 identificadores** — sem duplicata, desde 08/09 |

- 🔴 **As 7 urgentes do grupo A**, antes de 15/09: **D-216** (as atendentes aprovam — e o que não se afrouxa), **D-217** (avisar por exceção, não por aprovação), **D-182** (a vigilância renova em 26/09), **D-207** (o recorte do escopo), **D-206** (a triagem por CNJ), **D-209** (a janela que reabre) e **D-211** (matéria sensível)
- 🟠 **As 8 do grupo A que podem esperar:** D-149, D-201, D-198, D-148, D-192, **D-215** (HMAC — era D-159), D-196 e **D-218** (catálogo de gabaritos)
- ✅ **A colisão de numeração foi corrigida em 08/09, com o aval do usuário.** **D-157, D-158 e D-159** tinham dois significados cada — o Marco 3 (31/08) e a revisão externa (01/09). O trio da revisão externa virou **D-213, D-214 e D-215**; o do Marco 3 ficou. ⚠️ **E a §15 tinha a mesma doença:** o **R-70** nomeava dois riscos, e o do ruído de alerta (D-209) virou **R-76**. As citações neste documento, no plano e no digesto foram acertadas. **Terceira vez** que isso acontece — depois de D-101/D-102 e dos RF do PRD (P-18) —, e a prevenção em hook entrou no P-18
...
- ✅ **D-194 — RESPONDIDA em 05/09: pode.** O escritório recusou a proposta. Virou decisão formal com a faixa A4 dividida em A4a/A4b e quatro compensações; o custo assumido está em **R-68**. Sobrou uma pergunta nova: **20e** — quando a janela de reversão expira sem a advogada reagir, o alerta fecha ou reabre?
- ✅ **O N2 do rito ganhou destinatário proposto.** A pergunta **20b foi reescrita** em 05/09: sem hierarquia não há "sócia responsável", e o último degrau proposto passa a ser a advogada **Malu Souza** — não por organograma, mas porque é a responsável interna (D-200) e detém ~289 dos ~313 processos (D-193). Falta o "de acordo"
...
- 🔴 **Perguntas 24, 25, 26 e 27 (Trello) — as quatro subiram para bloqueadoras de E2 em 08/09 (D-226).** As 26 e 27 o escritório não sabe responder e viraram **levantamento técnico nosso**, dependente da chave de API; a 27 é a crítica, porque o Butler reage às nossas escritas e precisa ser inventariado **antes** da primeira gravação — agora com bloqueio **em código** (RF-62, R-81). As **24 e 25** (quantos quadros, e o fluxo típico de um card) **nunca foram enviadas ao escritório**: estavam em "escopo fino de E3", e sem elas não há onde criar o card de prazo nem para onde movê-lo (D-230)
